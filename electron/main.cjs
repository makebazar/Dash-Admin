...
const DEFAULT_SERVER_URL = "https://mydashadmin.ru";
...
function getDisplays() {
  ...
  return screen
    .getAllDisplays()
    .map((display, index) =>
      serializeDisplay(display, index, primaryDisplayId),
    );
}
...
function applyDisplayMode() {
  ...
  if (state.fullscreen) {
    ...
    const x =
      ...
      Math.round((targetDisplay.workArea.width - width) / 2);
    const y =
      targetDisplay.workArea.y +
      Math.round((targetDisplay.workArea.height - height) / 2);
...
function buildBootstrapPayload() {
  ...
  return {
    ...
    orientation: state.orientation === "portrait" ? "portrait" : "landscape",
    selectedDisplayId: state.selectedDisplayId
      ? String(state.selectedDisplayId)
      : null,
...
function applyRemoteDeviceState(device) {
  ...
  const nextState = writeStore({
    ...
    controlAction:
      device.controlAction === "pause"
        ? "pause"
        : device.controlAction === "jump"
          ? "jump"
          : null,
    controlSlideId: device.controlSlideId || null,
    controlUntil: device.controlUntil || null,
    controlUpdatedAt: device.controlUpdatedAt
      ? String(device.controlUpdatedAt)
      : null,
    serverUpdatedAt: device.serverUpdatedAt
      ? String(device.serverUpdatedAt)
      : null,
...
async function startRemoteStream() {
  ...
  try {
    const response = await fetch(
      `${serverUrl}/api/signage/device/stream?${params.toString()}`,
      {
        headers: buildRemoteHeaders(serverUrl, {
...
app.whenReady().then(async () => {
  ...
  // Strip restrictive headers to allow iframing ANY site
