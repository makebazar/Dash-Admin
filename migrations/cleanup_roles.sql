...
-- Ensure we have both employee roles (and keep their settings up-to-date)
...
-- Reassign users that reference non-employee roles to 'Админ' to avoid FK violations.
UPDATE users u
...
-- Remove unused roles, keep only employee roles (only if not referenced by users).
DELETE FROM roles r
