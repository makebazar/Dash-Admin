...
export function normalizeInventorySettings(
  raw: RawInventorySettings | null | undefined,
): InventorySettings {
  ...
  const allowedWarehouseIds = Array.isArray(
    ...
    : [];
  const employeeDiscountPercentRaw = Number(
    source.employee_discount_percent ?? 0,
  );
  ...
  const discountOverridesSource =
    ...
    typeof rawDiscountOverrides === "object" &&
    !Array.isArray(rawDiscountOverrides)
      ? rawDiscountOverrides
      : {};
  ...
  const cashboxEnabled = stockEnabled
    ? (source.cashbox_enabled ?? true)
    : false;
  const employeeStockOperationsEnabled = stockEnabled
    ? (source.employee_stock_operations_enabled ?? true)
    : false;
  const employeeWriteoffEnabled = stockEnabled
    ? (source.employee_writeoff_enabled ?? employeeStockOperationsEnabled)
    : false;
  const employeeTransferEnabled = stockEnabled
    ? (source.employee_transfer_enabled ?? employeeStockOperationsEnabled)
    : false;
  const reportReconciliationEnabled = cashboxEnabled
    ? (source.report_reconciliation_enabled ?? true)
    : false;
  const legacyCashboxWarehouseId = Number.isInteger(
  ...
  const cashboxWarehouseIdsBase = rawCashboxWarehouseIds
    .map((value) => Number(value))
    .filter((value) => Number.isInteger(value) && value > 0);
  const cashboxWarehouseIdsUnique = Array.from(
    new Set(cashboxWarehouseIdsBase),
  );
  const cashboxWarehouseIds =
    cashboxWarehouseIdsUnique.length > 0
      ? cashboxWarehouseIdsUnique
      : legacyCashboxWarehouseId
        ? [legacyCashboxWarehouseId]
        : [];
  const cashboxWarehouseId =
    cashboxWarehouseIds[0] ?? legacyCashboxWarehouseId ?? null;
  const handoverWarehouseId = Number.isInteger(
  ...
  const handoverWarehouseIdsBase = rawHandoverWarehouseIds
    .map((value) => Number(value))
    .filter((value) => Number.isInteger(value) && value > 0);
  const handoverWarehouseIdsUnique = Array.from(
    new Set(handoverWarehouseIdsBase),
  );
  const shiftAccountabilityMode =
    stockEnabled &&
    cashboxEnabled &&
    source.shift_accountability_mode === "WAREHOUSE"
      ? "WAREHOUSE"
      : "DISABLED";
  const handoverWarehouseIdsEffective =
    shiftAccountabilityMode === "WAREHOUSE"
      ? handoverWarehouseIdsUnique.length > 0
        ...
        : handoverWarehouseId
          ...
          : []
      : [];
  const handoverWarehouseIdEffective =
...
export function getShiftZoneLabel(
