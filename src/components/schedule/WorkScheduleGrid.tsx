...
function SortableRow({
  ...
  const shiftCount = useMemo(() => {
    ...
    const counts: Record<string, number> = { FULL: 0, DAY: 0, NIGHT: 0 };
    Object.entries(localSchedule[emp.id])
      .filter(([date]) => String(date).startsWith(prefix))
      .forEach(([, type]: any) => {
        const typeKey = type as string;
        if (counts.hasOwnProperty(typeKey)) {
          counts[typeKey as keyof typeof counts]++;
        }
      });

    return counts.FULL * (countFullAsTwo ? 2 : 1) + counts.DAY + counts.NIGHT;
    ...
    >
      ...
      >
        <div className="flex h-full min-h-[56px] items-center">
          ...
          <div
            className={cn(
              "flex flex-col flex-1 justify-center py-2 pr-4",
              emp.dismissed_at && "opacity-70 grayscale",
            )}
          >
          ...
          >
            <div
              className={cn(
                ...
                type === "DAY" &&
                  "bg-amber-500/10 text-amber-500 shadow-sm inset-y-0",
                type === "NIGHT" &&
                  "bg-indigo-500/10 text-indigo-400 shadow-sm inset-y-0",
                type === "FULL" &&
                  "bg-slate-900/5 text-slate-700 shadow-sm inset-y-0",
...
export function WorkScheduleGrid({
  clubId,
  month,
  year,
  initialData,
  ...
  const enqueueShiftUpdate = (
    cellId: string,
    payload: { userId: string; date: string; shiftType: string | null },
  ) => {
  ...
  const MobileScheduleList = () => (
    <div className="md:hidden space-y-3">
      {days.map((d) => {
        ...
        const dayWorkers = localEmployees.filter(
          (e: any) => localSchedule[e.id]?.[dateStr] === "DAY",
        );
        const nightWorkers = localEmployees.filter(
          (e: any) => localSchedule[e.id]?.[dateStr] === "NIGHT",
        );
        const fullWorkers = localEmployees.filter(
          (e: any) => localSchedule[e.id]?.[dateStr] === "FULL",
        );
                ...
                <div className="flex flex-wrap gap-1.5">
                  {dayWorkers.length > 0 ? (
                    dayWorkers.map((w: any) => (
                      <div
                        key={w.id}
                        className="inline-flex items-center rounded bg-amber-500/10 px-2 py-1 border border-amber-500/20"
                      >
                    ...
                    <span className="text-xs font-medium text-amber-500/50 italic">
                      Нет смен
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {nightWorkers.length > 0 ? (
                    nightWorkers.map((w: any) => (
                      <div
                        key={w.id}
                        className="inline-flex items-center rounded bg-indigo-500/10 px-2 py-1 border border-indigo-500/20"
                      >
                    ...
                    <span className="text-xs font-medium text-indigo-400/50 italic">
                      Нет смен
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {fullWorkers.length > 0 ? (
                    fullWorkers.map((w: any) => (
                      <div
                        key={w.id}
                        className="inline-flex items-center rounded bg-slate-900/5 px-2 py-1 border border-slate-200"
                      >
                    ...
                    <span className="text-xs font-medium text-slate-500/60 italic">
                      Нет смен
                    </span>
                  )}
                </div>
                ...
                >
                  <div className="flex flex-col gap-1">
                    <span
                      className={cn(
                        "text-xs font-bold",
                        coverage[d.dateStr].DAY > 0
                          ? "text-emerald-500"
                          : "text-muted-foreground/30",
                      )}
                    >
                      {coverage[d.dateStr].DAY}{" "}
                      <Sun className="inline h-3 w-3 -mt-0.5" />
                    </span>
                    <span
                      className={cn(
                        "text-xs font-bold",
                        coverage[d.dateStr].NIGHT > 0
                          ? "text-indigo-400"
                          : "text-muted-foreground/30",
                      )}
                    >
                      {coverage[d.dateStr].NIGHT}{" "}
                      <Moon className="inline h-3 w-3 -mt-0.5" />
                    </span>
                  </div>
                </td>
              );
      ...
      <div className="flex flex-col gap-3 border-t border-border bg-accent/30 p-4 text-sm sm:flex-row sm:flex-wrap sm:gap-8 sm:items-center sm:p-6">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-muted-foreground font-medium">
            ({clubSettings?.day_start_hour || "09"}:00 -{" "}
            {clubSettings?.night_start_hour || "21"}:00)
          </span>
        </div>
        <div className="flex items-center gap-2.5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Switch
            checked={countFullAsTwo}
            onCheckedChange={updateCountFullAsTwo}
          />
        </div>
        {!readOnly && (
          <div className="flex items-center gap-2.5">
