...
export function SignageStage({
  ...
  onCurrentSlideChangeAction?: (slide: SignageSlide | null) => void;
}) {
  ...
  const forcedSlide = useMemo(
    () =>
      forcedSlideId
        ? (activeSlides.find((slide) => slide.id === forcedSlideId) ?? null)
        : null,
  ...
  const targetSlide =
    (isForced ? forcedSlide : null) ?? activeSlides[activeIndex] ?? null;
  ...
  useEffect(() => {
    ...
    setActiveIndex((current) => {
      if (visibleSlide?.id) {
        const visibleIndex = activeSlides.findIndex(
  ...
  useEffect(() => {
    ...
    const forcedIndex = activeSlides.findIndex(
      (slide) => slide.id === forcedSlide.id,
    );
  ...
  useEffect(() => {
    onCurrentSlideChangeAction?.(visibleSlide);
  }, [visibleSlide, onCurrentSlideChangeAction]);
...
function SlideFrame({
  ...
  if (slide.mediaType === "website") {
    ...
    if (!isElectron) {
      return (
        <iframe
          className={cn(layerClassName, "bg-black border-0 overflow-hidden")}
          src={slide.imageUrl}
          style={{ animation, width: "100%", height: "100%", border: "none" }}
          onLoad={onReady}
          onError={onError}
    ...
    // We use <webview> for Electron to bypass all iframe restrictions (X-Frame-Options, CSP, JS frame busting)
    // For our Windows Signage app, <webview> is the professional choice.
    ...
    return (
      <Webview
        ...
        style={{ animation, width: "100%", height: "100%", border: "none" }}
        onLoad={() => onReady?.()}
        // Force ready after a delay just in case some sites block onload or events
        ref={(el: any) => {
          if (el) {
            // Electron specific: Wait for dom-ready if onload doesn't fire as expected
...
function EmptyStage({
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_top,rgba(59,130,246,0.12),transparent_28%),linear-gradient(180deg,rgba(255,255,255,0.02),transparent_30%)]">
      <div
        className={cn(
...
if (
  ...
  !document.getElementById("signage-stage-keyframes")
) {
