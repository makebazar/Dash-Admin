...
export function EmployeeSalesWizard({
  clubId,
  userId,
  activeShiftId,
  onExit,
}: EmployeeSalesWizardProps) {
  const [isPending, startTransition] = useTransition();
  const [allProducts, setAllProducts] = useState<Product[]>([]);
  const [receipts, setReceipts] = useState<ShiftReceipt[]>([]);
  const [salarySaleCandidates, setSalarySaleCandidates] = useState<
    SalarySaleCandidate[]
  >([]);
  ...
  const [paymentType, setPaymentType] =
    useState<ShiftReceiptPaymentType>("cash");
  ...
  const salaryPricing = useMemo(() => {
    const candidate = salarySaleCandidates.find(
  ...
  const [selectedReceipt, setSelectedReceipt] = useState<ShiftReceipt | null>(
    null,
  );
  ...
  const [topupPlayer, setTopupPlayer] = useState<{
    id: string;
    full_name: string;
    phone_number: string;
  } | null>(null);
  ...
  const handleWebSocketMessage = useCallback(
    (message: any) => {
      ...
      if (message.type === "PROMO_QUEUE_UPDATED") {
        getPromoQueue(clubId, userId).then(setPromoQueue).catch(console.error);
      }
      ...
      if (message.type === "PLAYER_CHECKIN" && message.player) {
        ...
        showMessage({
          title: "🔔 Гость подошел",
          description: `${message.player.full_name} сканировал QR на ресепшене`,
        });
      }
    },
    [activeShiftId, clubId, userId, showMessage],
  );
  ...
  useEffect(() => {
    setCart((prev) =>
      prev.map((item) => {
        const baseUnitPrice =
          paymentType === "salary"
            ? salaryPricing.priceMode === "COST"
              ? item.cost_price
              : item.selling_price
            : item.selling_price;
        const effectiveUnitPrice =
          paymentType === "salary" && salaryPricing.priceMode === "SELLING"
            ? roundMoney(
                baseUnitPrice * (1 - salaryPricing.discountPercent / 100),
              )
            : baseUnitPrice;
  ...
  const receiptTotalForShift = useMemo(() => {
    return receipts
      ...
      .reduce(
        (acc, r) =>
          acc +
          (Number(r.total_amount || 0) - Number(r.total_refund_amount || 0)),
        0,
      );
  }, [receipts]);

  const addToCart = useCallback(
    (product: any, quantityToAdd: number) => {
      ...
      setCart((prev) => {
        ...
        next[idx] = {
          ...next[idx],
          quantity: next[idx].quantity + quantityToAdd,
          price,
        };
        return next;
      });
  ...
  useEffect(() => {
    ...
    const handleGlobalKeydown = async (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter" && barcodeBuffer.length > 3) {
        ...
        // 1. Пробуем найти игрока по штрих-коду (UUID или телефон)
        const looksLikeUUID =
          /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(
            barcode,
          );
        const looksLikePhone = /^[0-9]{4,15}$/.test(barcode);

        if (looksLikeUUID || looksLikePhone) {
          try {
            const res = await fetch(
              `/api/promo/admin/players?clubId=${clubId}&phone=${barcode}`,
            );
            ...
            if (data.players && data.players.length > 0) {
              ...
              if (isTopupDialogOpen) {
                ...
                setSelectedPromoPlayer(player);
              }
              showMessage({
                title: isTopupDialogOpen
                  ? "👤 Игрок для пополнения"
                  : "👤 Гость выбран",
            ...
            console.error("Scanner player search error:", err);
          }
        }

        // 2. Ищем товар по штрих-коду
        try {
  ...
  const suggestions = useMemo(() => {
    ...
    for (const p of inStockProducts) {
      ...
      const score =
        (bestIdx === 0 ? 1000 : 0) + (bestVariantLen >= 3 ? 50 : 0) - bestIdx;
      hits.push({ p, score });
    }
  ...
  const handleInputEnter = async () => {
    ...
    if (looksLikeBarcode || looksLikeUUID) {
      if (looksLikeBarcode) {
      ...
      // Если не товар, пробуем найти игрока
      if (v.length >= 4) {
        setIsSearchingPromo(true);
        try {
          const res = await fetch(
            `/api/promo/admin/players?clubId=${clubId}&phone=${v}`,
          );
          const data = await res.json();
          if (data.players && data.players.length === 1) {
            setSelectedPromoPlayer(data.players[0]);
            setInputValue("");
            showMessage({
              title: "👤 Гость выбран",
              description: data.players[0].full_name,
            });
            return;
          } else if (data.players && data.players.length > 1) {
            setPromoSearchResults(data.players);
            setPromoSearchQuery(v);
            return;
          }
        } catch (e) {
      ...
      if (looksLikeBarcode) {
        ...
        if (product) {
          ...
          showMessage({
            title: "Не найдено",
            description: "Штрихкод не найден в каталоге",
          });
        }
      }
    }
    ...
    if (suggestions.length > 0) {
      const idx = Math.min(
        Math.max(selectedSuggestionIdx, 0),
  ...
  const removeCheckedInPlayer = (id: string) => {
    setCheckedInPlayers((prev) => prev.filter((p) => p.id !== id));
  };
  ...
  const finalizeReceipt = () => {
    ...
    if (paymentType === "salary") {
      if (!salaryTargetUserId) {
        showMessage({
          ...
          description:
            "Для продажи в счет ЗП нужно выбрать сотрудника с активной сменой",
        });
        return;
      }
      ...
      if (sum !== t) {
        showMessage({
          ...
          description: "Сумма наличных + карта должна равняться итогу",
        });
        return;
      }
    }
    ...
    startTransition(async () => {
      const result = await createShiftReceiptSafe(clubId, userId, {
        ...
        salary_target_user_id:
          paymentType === "salary" ? salaryTargetUserId : undefined,
        promo_player_id: selectedPromoPlayer?.id,
      });
      if (!result.ok) {
        showMessage({
          title: "Ошибка",
          description: result.error || "Ошибка пробития",
        });
        return;
      }
      try {
        ...
        showMessage({
          title: "Ошибка",
          description: e?.message || "Ошибка обновления кассы",
        });
      }
    });
  };
  ...
  const cancelReceipt = (id: number) => {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) {
        showMessage({
          title: "Ошибка",
          description: result.error || "Ошибка отмены",
        });
        return;
      }
      ...
      try {
        ...
        showMessage({
          title: "Ошибка",
          description: e?.message || "Ошибка отмены",
        });
      }
    });
  };
  ...
  const handleReturnReceipt = () => {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) {
        showMessage({
          title: "Ошибка",
          description: result.error || "Ошибка возврата",
        });
        return;
      }
      ...
      try {
        ...
        showMessage({
          title: "Ошибка",
          description: e?.message || "Ошибка возврата",
        });
      }
    });
  };
      ...
      if (!isInInput) {
        ...
        if (e.key === "ArrowUp" || e.key === "ArrowDown") {
          ...
          const idx = cart.findIndex(
            (i) => i.product_id === selectedCartProductId,
          );
          const nextIdx =
            e.key === "ArrowUp"
  ...
  useEffect(() => {
    ...
    if (
      selectedCartProductId &&
      cart.some((i) => i.product_id === selectedCartProductId)
    )
      return;
  ...
  return (
    <>
      <div className="h-dvh bg-zinc-950 text-foreground flex flex-col font-sans selection:bg-primary/20 overflow-hidden">
          ...
          {!activeShiftId ? (
            <div className="flex flex-col items-center justify-center h-full text-center animate-in fade-in duration-500">
              ...
              <p className="text-muted-foreground max-w-sm mx-auto">
                Откройте смену, чтобы начать принимать оплаты и списывать
                товары.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both min-h-0">
                ...
                <section className="shrink-0 space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    {suggestions.length > 0 && inputValue.trim() !== "" && (
                      <div className="absolute z-10 w-full mt-2 rounded-2xl bg-zinc-900 border border-zinc-800 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 max-h-75 overflow-y-auto custom-scrollbar">
                        {suggestions.map((p, idx) => (
                          ...
                          >
                            ...
                            <span className="text-muted-foreground shrink-0 font-mono text-lg">
                        ...
                        onClick={() => {
                          startTransition(async () => {
                            const ok = await confirmAction({
                              title: "Очистить чек",
                              description:
                                "Вы уверены, что хотите очистить текущий чек?",
                        ...
                        <div className="space-y-1">
                          {cart.map((i) => (
                            <div
                              key={i.product_id}
                              className={cn(
                                "group flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl transition-colors cursor-pointer gap-3",
                                i.product_id === selectedCartProductId
                                  ? "bg-zinc-800/80"
                                  : "hover:bg-zinc-800/40",
                              )}
                              onClick={() =>
                                setSelectedCartProductId(i.product_id)
                              }
                            >
                              ...
                              <div className="flex items-center gap-3 shrink-0 self-end sm:self-auto">
                                <div className="flex items-center bg-zinc-950 rounded-lg border border-zinc-800/50 overflow-hidden h-8">
                                  <button
                                    className="w-8 h-full flex items-center justify-center text-muted-foreground hover:bg-zinc-800 hover:text-white transition-colors"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      updateCartQty(
                                        i.product_id,
                                  ...
                                  <div className="w-8 text-center font-medium font-mono text-xs">
                                    {i.quantity}
                                  </div>
                                  <button
                                    className="w-8 h-full flex items-center justify-center text-muted-foreground hover:bg-zinc-800 hover:text-white transition-colors"
                                ...
                                >
                                  <div className="flex min-w-0 flex-col gap-1 pr-4">
                                    ...
                                    <span className="text-xs text-zinc-400">
                                      {candidate.role} · доступно{" "}
                                      {candidate.available_amount.toLocaleString()}{" "}
                                      ₽
                                    </span>
                                  </div>
                                </SelectItem>
              ...
              <div className="lg:col-span-4 flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-1">
                {/* Promo Queue Section */}
                {promoQueue.length > 0 && (
                  <section className="flex flex-col gap-3 animate-in fade-in slide-in-from-right-4 duration-500">
                    ...
                    <div className="space-y-3">
                      {promoQueue.map((item) => (
                        <div
                          key={item.id}
                          className="p-4 rounded-3xl border border-emerald-500/20 bg-emerald-500/5 space-y-4"
                        >
                          <div className="flex items-start justify-between">
                            <div className="min-w-0">
                              <div className="font-bold text-sm text-emerald-500 truncate">
                                {item.prize_name}
                              </div>
                            ...
                            <Badge className="bg-emerald-500/20 text-emerald-500 border-emerald-500/30 text-[10px] uppercase font-black">
                              Ожидает
                            </Badge>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 rounded-xl text-[10px] font-bold transition-all border border-zinc-800"
                            >
                          ...
                          <Button
                            ...
                            disabled={isPending}
                            className="w-full h-10 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold text-xs"
                          >
                            {isPending ? (
                      ...
                      {receipts.length === 0 ? (
                        ...
                        receipts
                          ...
                          .map((r) => (
                            <div
                              key={r.id}
                              className="p-4 rounded-2xl border border-zinc-800/50 bg-zinc-950 flex flex-col gap-3 transition-colors hover:border-zinc-700"
                            >
                                ...
                                <div className="pt-3 border-t border-zinc-800/50 space-y-2">
                                  {r.items.map((it: any) => {
                                    const isFullyReturned =
                                      (it.available_qty || 0) <= 0;
                                    const returnedQty = it.returned_qty || 0;

                                    return (
                                      <div
                                        key={it.id}
                                        className="flex justify-between items-center text-xs text-muted-foreground group/item"
                                      >
        ...
        <DialogContent className="bg-card border-border sm:max-w-md rounded-3xl p-6">
          <DialogHeader className="mb-4">
            ...
            <DialogDescription className="text-base">
              {
                selectedReceipt?.items?.find((i: any) => i.id === returnItemId)
                  ?.product_name
              }
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <Input
                ...
                min="1"
                max={
                  selectedReceipt?.items?.find(
                    (i: any) => i.id === returnItemId,
                  )?.available_qty
                }
