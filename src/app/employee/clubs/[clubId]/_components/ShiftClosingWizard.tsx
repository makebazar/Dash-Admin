...
import {
  StockCountWorkspace,
  type StockCountWorkspaceItem,
} from "@/app/clubs/[clubId]/inventory/_components/StockCountWorkspace";
  ...
  for (const receipt of receipts) {
    ...
    for (const item of receipt.items || []) {
      const netQty = Math.max(
        0,
        Number(item.quantity) - Number(item.returned_qty || 0),
      );
...
function summarizeInventory(items: ExtendedInventoryItem[]): InventorySummary {
  ...
  for (const item of items) {
    ...
    const difference =
      Number(item.actual_stock) - Number(item.expected_stock || 0);
    ...
    if (difference < 0) {
      shortageItems += 1;
      shortageValue +=
        Math.abs(difference) * Number(item.selling_price_snapshot || 0);
    } else {
      excessItems += 1;
...
export function ShiftClosingWizard({
  ...
  const [selectedUnaccountedProduct, setSelectedUnaccountedProduct] =
    useState("");
  ...
  const notifyError = useCallback(
    (description: string, title = "Ошибка") => {
  ...
  const notifyInfo = useCallback(
    (description: string, title = "Уведомление") => {
      showMessage({ title, description });
    },
    [showMessage],
  );
  ...
  const usesSalesReconciliation =
    !isStartShiftMode &&
    isShiftSalesMode &&
    Boolean(inventorySettings?.report_reconciliation_enabled);
  const usesInventoryCounting =
    isStartShiftMode || (!isStartShiftMode && !skipInventory);
  const usesRevisionMode = !isStartShiftMode && !isShiftSalesMode;
  const usesShiftInventoryReconciliation =
    usesSalesReconciliation && usesInventoryCounting;
  const inventoryStep = isStartShiftMode ? 1 : isShiftSalesMode ? 3 : 2;
  const finalizeStep = isStartShiftMode ? 2 : isShiftSalesMode ? 4 : 3;
  const totalSteps = isStartShiftMode
    ? 2
    : isShiftSalesMode
      ? skipInventory
        ? 2
  ...
  const handleSSEMessage = useCallback(
    (message: any) => {
      if (
        message.type === "RECEIPT_CREATED" ||
        message.type === "RECEIPT_VOIDED" ||
        message.type === "RECEIPT_ITEM_RETURNED"
      ) {
        // Обновляем чеки при создании/аннулировании
        if (activeShiftId) {
          getShiftReceipts(clubId, userId, String(activeShiftId), {
            includeVoided: true,
          })
  ...
  const salesPreview = useMemo(() => {
    ...
    const standardSales = inventoryItems
      .filter(
        (i) =>
          i.actual_stock !== null &&
          (i.expected_stock || 0) > (i.actual_stock || 0),
      )
      .map((i) => ({
        id: i.id,
        name: i.product_name,
  ...
  const discrepancyItems = useMemo(() => {
    return inventoryItems
      ...
      .sort((a, b) => {
        if (Math.sign(a.difference) !== Math.sign(b.difference))
  ...
  const hasInventoryMismatch =
    usesShiftInventoryReconciliation && inventorySummary?.isPerfect === false;
  ...
  const canProceedFromStep1 = useMemo(() => {
    ...
    isStartShiftMode,
    requiredChecklist?.items?.length,
    revenueKey,
    stepOneMissingFields.length,
    usesSalesReconciliation,
  ]);

  const stepOneBlockReasons = useMemo(() => {
    const reasons: string[] = [];
    if (stepOneMissingFields.length > 0) {
      const schema = Array.isArray(reportTemplate?.schema)
        ? reportTemplate.schema
        : [];
    ...
    requiredChecklist?.items?.length,
    revenueKey,
    stepOneMissingFields,
    usesSalesReconciliation,
  ]);

  const shiftCommentRequired = useMemo(() => {
    if (isStartShiftMode) return false;
    if (usesSalesReconciliation && step === 2)
      return reconcileSnapshot.diff !== 0;
    if (step === finalizeStep && calculationResult)
      return calculationResult.diff !== 0;
    return false;
  }, [
    calculationResult,
    finalizeStep,
    isStartShiftMode,
    reconcileSnapshot.diff,
    step,
    usesSalesReconciliation,
  ]);
  ...
  const reconcileBlockReason = useMemo(() => {
    ...
    if (shiftCommentRequired && !hasShiftComment)
      return "Укажите причину расхождения";
    return null;
  }, [
    hasShiftComment,
    isPending,
  ...
  const finalizeBlockReason = useMemo(() => {
    ...
    if (forgottenItems.length > 0)
      return `Не посчитано товаров: ${forgottenItems.length}`;
    if (shiftCommentRequired && !hasShiftComment)
      return "Укажите причину расхождения";
    return null;
  }, [
  ...
  const finalizeSummary = useMemo(() => {
    ...
    if (usesSalesReconciliation) {
      ...
      if (calculationResult.diff === 0 && hasInventoryMismatch) {
        return {
          ...
          value: `${inventorySummary?.discrepancyQuantity ?? 0} шт.`,
          description:
            "Продажи по смене сошлись, но ревизия нашла расхождения по остаткам.",
        };
      }
    ...
    isStartShiftMode,
    usesSalesReconciliation,
    usesShiftInventoryReconciliation,
  ]);
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      try {
        const receipts = await getShiftReceipts(clubId, userId, shiftIdStr, {
          includeVoided: true,
        });
  ...
  useEffect(() => {
    ...
    autosaveTimeoutRef.current = setTimeout(() => {
      ...
      startTransition(async () => {
        try {
          const result = await bulkUpdateInventoryItemsSafe(
            itemsToSave,
            clubId,
          );
          if (!result.ok) {
            console.error(
              "Failed to autosave inventory progress:",
              result.error,
            );
            return;
          }
  ...
  const handleStep1Submit = () => {
    ...
    if (requiredChecklist?.items?.length && !isRequiredChecklistComplete) {
      notifyInfo(
        "Пройдите чеклист перед закрытием смены",
        "Чеклист обязателен",
      );
    ...
    if (!isStartShiftMode && usesSalesReconciliation && !revenueKey) {
      notifyError(
        "В настройках склада не выбрана метрика выручки по умолчанию. Без нее нельзя включить сверку итогов.",
        "Настройте метрику",
      );
      return;
    }
  ...
  const removePhoto = (itemId: number, urlToRemove: string) => {
    setChecklistResponses((prev) => ({
      ...
      [itemId]: {
        ...prev[itemId],
        photo_urls: (prev[itemId]?.photo_urls || []).filter(
          (url) => url !== urlToRemove,
        ),
      },
    }));
  ...
  const toggleProblematicWorkstation = (
    itemId: number,
    wsId: string,
    maxScore: number,
    targetWs: any[],
    ...
    setProblematicItems((prev) => {
      ...
      const workstationNames = targetWs
        .filter((w) => newItems.includes(w.id))
        .map((w) => w.name);
  ...
  const startInventory = () => {
    ...
    startTransition(async () => {
      try {
        ...
        if (!targetMetric) {
          notifyError(
            "В настройках склада не выбрана метрика выручки по умолчанию.",
            "Настройте метрику",
          );
          return;
        }
  ...
  const handleBarcodeScan = useCallback(
    async (barcode: string) => {
      ...
      const item = inventoryItems.find(
        (i) =>
          i.barcode === barcode || (i.barcodes && i.barcodes.includes(barcode)),
      );
      ...
      try {
        ...
        if (product) {
          ...
          const existingInList = inventoryItems.find(
          ...
          const currentCounts = inventoryItems.reduce(
            (acc, i) => {
          ...
          const addResult = await addProductToInventorySafe(
            inventoryId!,
            product.id,
          );
          if (!addResult.ok) {
          ...
          setInventoryItems(
            invItems.map((i) => {
              ...
              return {
                ...
                actual_stock: isNew
                  ? 1
                  : (currentCounts[i.id] ?? i.actual_stock),
  ...
  const handleInventoryWorkspaceChange = useCallback(
    (nextItems: StockCountWorkspaceItem[]) => {
      setInventoryItems((prev) =>
        prev.map((item) => {
          const next = nextItems.find(
            (candidate) => candidate.id === String(item.id),
          );
          if (!next) return item;
          return {
            ...item,
            ...
            last_modified:
              next.countedQuantity !== item.actual_stock
                ? Date.now()
                : item.last_modified,
          };
        }),
  ...
  const handleReconcileNext = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const nextCalculatedRevenue = nextSalesPreview.reduce(
    ...
    startTransition(async () => {
      try {
        ...
        if (itemsToUpdate.length > 0) {
          const saveResult = await bulkUpdateInventoryItemsSafe(
            itemsToUpdate,
            clubId,
          );
          if (!saveResult.ok) {
        ...
        if (isStartShiftMode) {
          const refreshedItems = inventoryId
            ? await getInventoryItems(inventoryId)
            : inventoryItems;
        ...
        const standardCalculatedRev = inventoryItems.reduce((acc, item) => {
          if (
            ...
            (item.expected_stock || 0) > (item.actual_stock || 0)
          ) {
        ...
        const unaccountedRev = unaccountedSales.reduce(
  ...
  const handleFinalize = () => {
    ...
    if (forgottenItems.length > 0) {
      notifyInfo(
        `Вы не посчитали ${forgottenItems.length} товаров. Укажите их остаток (даже если 0), чтобы закрыть смену.`,
    ...
    startTransition(async () => {
      try {
        if (isStartShiftMode) {
          const result = await closeInventorySafe(inventoryId, clubId, 0, [], {
            salesRecognition: "NONE",
          });
          if (!result.ok) {
  ...
  const addUnaccountedSale = () => {
    const product = allProducts.find(
      (p) => p.id === Number(selectedUnaccountedProduct),
    );
    ...
    if (inInventory) {
      notifyInfo(
        "Этот товар уже есть в списке инвентаризации. Просто укажите его остаток там.",
        "Товар уже добавлен",
      );
  ...
  return createPortal(
    <div className="fixed inset-0 h-[100dvh] bg-slate-950 text-primary-foreground flex flex-col z-[9999] overflow-hidden overscroll-none">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-orange-100">
                        Чеклист: {requiredChecklist.name}
                      </h4>
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-5">
                {reportTemplate?.schema.map((field: any, idx: number) => {
                  const isMissing = stepOneMissingFields.includes(
                    field.metric_key,
                  );
                  return (
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() => {
                                  const currentList = normalizeExpenseEntries(
                          ...
                          <Input
                            ...
                            className={cn(
                              ...
                              isMissing
                                ...
                                : "border-slate-800 focus:ring-purple-500",
                            )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  ...
                  <div className="text-xl font-bold mt-1">
                    {reconcileSnapshot.reported.toLocaleString()} ₽
                  </div>
                ...
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  ...
                  <div className="text-xl font-bold mt-1 text-blue-400">
                    {reconcileSnapshot.calculated.toLocaleString()} ₽
                  </div>
                </div>
              </div>
                ...
                <div className="flex-1">
                  <div className="font-bold flex justify-between items-center">
                    ...
                    <span className="text-xl font-black">
                      {reconcileSnapshot.diff > 0 ? "+" : ""}
                      {reconcileSnapshot.diff.toLocaleString()} ₽
                    </span>
                  </div>
                  <p className="text-xs opacity-80 mt-1 leading-relaxed">
                    {reconcileSnapshot.diff === 0
                      ? "Сумма в отчете соответствует пробитым товарам."
                      : reconcileSnapshot.diff > 0
                        ? "В отчете больше денег, чем по пробитым товарам. Возможно, не пробили часть продаж."
                    ...
                    <Table>
                      <TableBody>
                        {salesPreview.map((s, idx) => (
                          <TableRow
                            key={`${s.id}-${idx}`}
                            className="border-slate-800 hover:bg-primary/90/30"
                          >
                            <TableCell className="py-3">
                              <div className="flex flex-col">
                                <div className="flex items-center gap-1.5">
                                  {s.isUnaccounted && (
                                    <span className="bg-blue-500/20 text-blue-400 text-[8px] px-1 rounded uppercase font-bold">
                                      Ручн.
                                    </span>
                                  )}
              ...
              {reconcileSnapshot.diff !== 0 && (
                <div className="space-y-3">
                  <Label
                    className={cn(
                      "text-xs uppercase tracking-wider ml-1",
                      shiftCommentRequired && !hasShiftComment
                        ? "text-red-400"
                        : "text-muted-foreground/70",
                    )}
                  >
        ...
        {step === inventoryStep && (
          <div className="space-y-6 max-w-4xl mx-auto pb-20">
            <StockCountWorkspace
              title={
                isStartShiftMode ? "Стартовый пересчет" : "Пересчет товаров"
              }
              description={
                isStartShiftMode
                ...
                <>
                  ...
                  >
                    <RefreshCcw
                      className={cn(
                        "mr-2 h-4 w-4",
                        (isPending || isRefreshingCatalog) && "animate-spin",
                      )}
                    />
                    Синхронизировать
                  </Button>
        ...
        {step === finalizeStep && calculationResult && (
          <div className="space-y-6 max-w-2xl mx-auto pb-20">
            {isStartShiftMode ? (
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  ...
                  <div className="text-xl font-bold mt-1 text-blue-400">
                    {inventorySummary?.discrepancyItems ?? 0}
                  </div>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  ...
                  <div className="text-xl font-bold mt-1">
                    {calculationResult.reported.toLocaleString()} ₽
                  </div>
                </div>
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  <span className="text-muted-foreground text-[10px] uppercase font-bold tracking-wider">
                  ...
                  <div className="text-xl font-bold mt-1 text-blue-400">
                    {calculationResult.calculated.toLocaleString()} ₽
                  </div>
                </div>
              </div>
            )}
            ...
            {usesShiftInventoryReconciliation && inventorySummary && (
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  ...
                  <div className="text-xl font-bold mt-1">
                    {inventorySummary.discrepancyItems}
                  </div>
                  <div className="text-[10px] text-muted-foreground mt-1">
                ...
                <div className="bg-primary/50 p-4 rounded-2xl border border-slate-800">
                  <span className="text-muted-foreground text-[10px] uppercase font-bold tracking-wider">
                    Отклонение по складу
                  </span>
                  <div className="text-xl font-bold mt-1 text-blue-400">
                    {inventorySummary.discrepancyValue.toLocaleString()} ₽
                  </div>
                  <div className="text-[10px] text-muted-foreground mt-1">
                    Недостача: {inventorySummary.shortageItems} · Излишки:{" "}
                    {inventorySummary.excessItems}
                  </div>
                </div>
              </div>
            )}
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-red-500/20">
                      <div>
                        ...
                        <p className="text-[10px] text-red-200/70">
                          Какие позиции оказались ниже ожидания
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-blue-500/20">
                      <div>
                        ...
                        <p className="text-[10px] text-blue-200/70">
                          Какие позиции оказались выше ожидания
                        </p>
                      </div>
                      <Badge
                        variant="outline"
              ...
              <div className="bg-red-900/20 border border-red-500/30 p-5 rounded-2xl space-y-4">
                <div className="flex items-center gap-3 text-red-400">
                  ...
                  <h4 className="font-bold">
                    Вы не посчитали {forgottenItems.length} товаров!
                  </h4>
                </div>
                <p className="text-xs text-red-300/80 leading-relaxed">
                  Эти товары числятся на складе, но вы не указали их фактическое
                    ...
                    >
                      ...
                      {!inventorySettings?.blind_inventory_enabled && (
                        <span className="text-muted-foreground italic">
                          Ожидалось: {item.expected_stock} шт.
                        </span>
                      )}
                    </div>
                  ))}
            ...
            {usesRevisionMode &&
              inventoryItems.some(
                (item) =>
                  (item.expected_stock || 0) === 0 &&
                  (item.actual_stock || 0) > 0,
              ) && (
                  ...
                  <div className="flex items-center gap-3 text-emerald-400">
                    ...
                    <div>
                      <h4 className="font-bold text-lg">
                        Доступна выплата: {payoutSuggestion.amount} ₽
                      </h4>
                      <p className="text-xs text-emerald-300/80">
                        Вы заработали эту сумму за смену (деньщина).
                      </p>
                    </div>
                  </div>
                    ...
                    payoutSuggestion.amount ? (
                      <div className="text-center space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground/70">
                          В кассе {reportData.cash_income || 0} ₽, а к выплате{" "}
                          {payoutSuggestion.amount} ₽. Выплата будет зачислена
                          на баланс.
                        </p>
                        <Button
                          disabled
                      ...
                      <>
                        <p className="text-sm text-slate-300 mb-2">
                          Забрать деньги из кассы сейчас?
                        </p>
                        <div className="flex gap-3">
                          <Button
                            onClick={() =>
                              setReportData({
                                ...reportData,
                                auto_payout_amount: payoutSuggestion.amount,
                              })
                            }
                            variant={
        ...
        {step === 1 && !isStartShiftMode && (
          <div className="space-y-2">
            <Button
              ...
              className="w-full h-14 text-lg font-bold bg-purple-600 hover:bg-purple-700 rounded-2xl shadow-lg shadow-purple-900/20 disabled:opacity-40 disabled:pointer-events-none"
            >
              {primaryStepOneLabel}
          ...
          <div className="space-y-2">
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="flex-1 h-14 text-lg font-bold bg-blue-600 hover:bg-blue-700 rounded-2xl shadow-lg shadow-blue-900/20 disabled:opacity-40 disabled:pointer-events-none"
              >
                {reconcileNextLabel}
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
        ...
        {step === inventoryStep && !usesSalesReconciliation && (
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={isPending}
              className="flex-1 h-14 text-lg font-bold bg-blue-600 hover:bg-blue-700 rounded-2xl shadow-lg shadow-blue-900/20"
            >
              {inventoryNextLabel}
              <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
        ...
        {step === finalizeStep && (
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={!canFinalizeShift}
              className="flex-1 h-14 text-lg font-bold bg-green-600 hover:bg-green-700 rounded-2xl shadow-lg shadow-green-900/20 disabled:opacity-40 disabled:pointer-events-none"
            >
        ...
        <DialogContent className="bg-slate-950 border-slate-800 text-primary-foreground max-w-[90vw] rounded-2xl">
          ...
          <DialogFooter className="flex-row gap-3">
            <Button
              ...
              className="flex-1 border-slate-800 h-12 rounded-xl"
            >
              Отмена
            </Button>
            <Button
              onClick={() => handleAddProductManually()}
              disabled={!selectedProductToAdd || isPending}
              className="flex-1 bg-blue-600 h-12 rounded-xl"
            >
              Добавить
            </Button>
          </DialogFooter>
      ...
      <Dialog
        open={isUnaccountedDialogOpen}
        onOpenChange={setIsUnaccountedDialogOpen}
      >
        <DialogContent className="bg-slate-950 border-slate-800 text-primary-foreground max-w-[90vw] rounded-2xl">
          <DialogHeader>
          ...
          <div className="py-6 space-y-4">
            <div className="space-y-2">
              ...
              <Select
                value={selectedUnaccountedProduct}
                onValueChange={setSelectedUnaccountedProduct}
              >
                <SelectTrigger className="bg-primary border-slate-800 h-12 rounded-xl">
                  <SelectValue placeholder="Выберите товар..." />
                </SelectTrigger>
          ...
          <DialogFooter className="flex-row gap-3">
            <Button
              variant="outline"
              onClick={() => setIsUnaccountedDialogOpen(false)}
              className="flex-1 border-slate-800 h-12 rounded-xl"
            >
              Отмена
            </Button>
            <Button
              onClick={addUnaccountedSale}
              disabled={!selectedUnaccountedProduct || !unaccountedQty}
              className="flex-1 bg-blue-600 h-12 rounded-xl"
            >
              Добавить
            </Button>
