...
async function fetchShiftAccountabilityStatus(
  clubId: string,
): Promise<ShiftAccountabilityStatusResponse> {
  const res = await fetch(
    `/api/employee/clubs/${clubId}/shift-accountability`,
    { cache: "no-store" },
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(
      data.error || "Не удалось загрузить статус сменной ответственности",
    );
  }
  return data as ShiftAccountabilityStatusResponse;
}

async function fetchHasOpenZoneSnapshot(
  clubId: string,
  shiftId: string | number,
) {
  const res = await fetch(
    `/api/employee/clubs/${clubId}/shifts/${shiftId}/open-snapshot`,
    { cache: "no-store" },
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
...
async function fetchEmployeeRequests(clubId: string) {
  const res = await fetch(`/api/employee/clubs/${clubId}/requests`, {
    cache: "no-store",
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
...
export default function EmployeeClubPage({
  params,
}: {
  ...
  const [hasShiftAccountability, setHasShiftAccountability] = useState<
    boolean | null
  >(null);
  const [hasOpenZoneSnapshotDraft, setHasOpenZoneSnapshotDraft] =
    useState(false);
  ...
  >([]);
  const [selectedShiftRoleId, setSelectedShiftRoleId] = useState<number | null>(
    null,
  );
  const normalizedInventorySettings = useMemo(
    () => normalizeInventorySettings(club?.inventory_settings),
    [club?.inventory_settings],
  );
  ...
  const canUseShiftReport =
    isEmployeeAccessLoaded && shiftEndMode !== "NO_REPORT";
  const canUseInventoryActions =
    isEmployeeAccessLoaded &&
    employeeSettings.inventory_actions_enabled !== false;
  const canUseShiftZoneHandover =
    isEmployeeAccessLoaded &&
    employeeSettings.shift_zone_handover_enabled !== false;
  const canUseEmployeeChecklists =
    isEmployeeAccessLoaded &&
    (employeeSettings.handover_checklist_on_start !== "DISABLED" ||
      employeeSettings.closing_checklist_enabled === true);
  const canStartShift =
    isEmployeeAccessLoaded && employeeSettings.shift_start_enabled !== false;

  const isSuppliesEnabled =
    normalizedInventorySettings.supplies_enabled && canUseInventoryActions;
  ...
  const canUseEmployeeWriteOff =
    isStockEnabled && normalizedInventorySettings.employee_writeoff_enabled;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as {
        items?: Array<{ counted_quantity?: number | null }>;
        selected_handover_source_shift_id?: string | null;
      };
      ...
      setHasOpenZoneSnapshotDraft(
        Boolean(parsed.selected_handover_source_shift_id) ||
          hasItems ||
          hasAnyCount,
      );
    } catch (error) {
  ...
  }, [activeShift?.id, canUseShiftZoneHandover, clubId]);

  useEffect(() => {
    if (
      !canUseShiftZoneHandover ||
      !clubId ||
      !activeShift?.id ||
  ...
  const updateSelectedShiftRole = useCallback(
    async (nextRoleId: number) => {
      ...
      try {
        ...
        const accessRes = await fetch(`/api/employee/clubs/${clubId}/me`, {
          cache: "no-store",
        });
        ...
        if (accessRes.ok) {
          ...
          const localCanUseEmployeeChecklists =
            settings.handover_checklist_on_start !== "DISABLED" ||
            settings.closing_checklist_enabled === true;
        ...
        const kpiRes = await fetch(`/api/employee/clubs/${clubId}/kpi`, {
          cache: "no-store",
        });
  ...
  const fetchData = async (id: string) => {
    try {
      ...
      const accessRes = await fetch(`/api/employee/clubs/${id}/me`, {
        cache: "no-store",
      });
      const accessJson = await accessRes.json().catch(() => ({}));
      if (accessRes.ok) setEmployeeAccess(accessJson);
      const localSettings = accessRes.ok ? accessJson?.settings || {} : {};
      const localCanUseShiftReport =
        localSettings.shift_end_mode !== "NO_REPORT";
      const localCanUseInventoryActions =
        localSettings.inventory_actions_enabled !== false;
      const localCanUseEmployeeChecklists =
        localSettings.handover_checklist_on_start !== "DISABLED" ||
        localSettings.closing_checklist_enabled === true;
      ...
      const results = await Promise.all([
        ...
        fetch(
          `/api/clubs/${id}/equipment/maintenance?assigned=me&status=PENDING,IN_PROGRESS`,
        ).then((r) => r.json()),
        fetch(
      ...
      if (tasksJson?.tasks) {
        ...
        if (localCanUseInventoryActions) {
          ...
          setClubTasks(
            tasks.filter((t: any) => t?.type === "EQUIPMENT_TRANSFER"),
          );
        }
      } else {
        setClubTasks([]);
  ...
  async function fetchShiftRoles(id: string) {
    try {
      const res = await fetch(`/api/employee/clubs/${id}/roles`, {
        cache: "no-store",
      });
      const data = await res.json();
      if (!res.ok) return;
      ...
      const activeId =
        data.active_role_id === null || data.active_role_id === undefined
          ? null
          : Number(data.active_role_id);
      const defaultId =
        data.default_role_id === null || data.default_role_id === undefined
          ? null
          : Number(data.default_role_id);
      const next =
        (Number.isFinite(activeId) ? activeId : null) ??
        (Number.isFinite(defaultId) ? defaultId : null) ??
        normalized[0]?.id ??
        null;
      ...
      if (res.ok) {
        ...
        if (canUseShiftZoneHandover) {
          const accountabilityStatus =
            await fetchShiftAccountabilityStatus(clubId);
          hasShiftZones =
  ...
  const handleStartShift = useCallback(async () => {
    ...
    const requiredHandover = shouldBlock
      ? checklistTemplates.find(
          (t: any) =>
            t.type === "shift_handover" && t.settings?.block_shift_open,
        )
      : null;
  ...
  const finalizeShiftClose = useCallback(
    async (data: any) => {
  ...
  const executeCompleteClubTask = useCallback(
    async (taskId: string) => {
      ...
      try {
        ...
        if (res.ok) {
          setClubTasks((prev) =>
            prev.filter((t: any) => String(t.id) !== taskId),
          );
        } else {
          const data = await res.json();
  ...
  const submitEndShift = async (data: any) => {
    ...
    try {
      ...
      alert(
        `Ошибка завершения смены: ${error instanceof Error ? error.message : "Неизвестная ошибка"}`,
      );
    } finally {
      setIsActionLoading(false);
    }
  ...
  const handleShiftClosingWizardComplete = useCallback(
    async (data: any) => {
