  ...
  const fetchUserData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Check if user has manager clubs based on permissions
        const managerClubs = (data.employeeClubs || []).filter(
          (club: Club) => club.can_access_management === true,
        );
    ...
    >
          ...
          <div className="flex items-center gap-1.5">
            {(hasOwnedClubs || hasManagerClubs) && (
              <Link href="/dashboard">
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-8 text-xs text-amber-400 hover:text-amber-300 hover:bg-amber-400/10 flex items-center gap-2"
                >
                  <Crown className="h-3.5 w-3.5" />
                  <span className="hidden sm:inline">
                    {hasOwnedClubs ? "Кабинет владельца" : "Управ кабинет"}
                  </span>
                </Button>
              </Link>
            )}
            <Link href="/support">
              <Button
                variant="ghost"
                size="sm"
                className="h-8 w-8 px-0 text-muted-foreground hover:text-foreground hover:bg-white/5"
              >
      ...
      {hasExpiredSubscription && (
        <div className="fixed top-14 left-0 right-0 z-40 border-b border-amber-500/20 bg-amber-500/10 px-4 py-2 text-xs font-medium text-amber-200 backdrop-blur-md flex items-center justify-center">
          ...
          {expiredClubNames.length ? `: ${expiredClubNames.join(", ")}` : ""}.
          Отправка отчетов и рабочие действия доступны.
        </div>
      )}

      <div
        className={cn(
          "flex flex-1 relative pt-14",
          hasExpiredSubscription && "mt-8",
        )}
      >
        {/* Mobile Overlay */}
        {isMobileMenuOpen && (
            ...
            <div className="flex h-full flex-col">
              ...
              <div className="flex-1 overflow-auto px-3 pb-4">
                <nav className="space-y-6">
                  {clubs.map((club) => {
                    const isClubActive = pathname.startsWith(
                      `/employee/clubs/${club.id}`,
                    );
