...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ clubId: string }> }) {
    try {
        ...
        if (!botName) return NextResponse.json({ error: "MAX_BOT_NAME is not set" }, { status: 500 })

        const code = uuidv4().replace(/-/g, "")
        const expiresAt = new Date(Date.now() + 15 * 60 * 1000).toISOString()

        await query(
            `INSERT INTO max_link_codes (club_id, code, created_by, expires_at)
             VALUES ($1, $2, $3, $4)`,
        ...
        return NextResponse.json({
            ...
            url: `https://max.ru/${botName}?start=${code}`,
        })
    } catch (error) {
