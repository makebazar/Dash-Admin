...
export async function GET(
  ...
  try {
    ...
    const warehousesRes = await query(
      `SELECT id, name FROM warehouses WHERE club_id = $1 ORDER BY name`,
      [clubId],
    );
    ...
    return NextResponse.json({
      club: {
        ...
        inventory_settings: normalizeInventorySettings(
          result.rows[0]?.inventory_settings,
        ),
      },
...
export async function PATCH(
  ...
  try {
    ...
    if (!hasModuleAccess(access, "settings_general", "edit", clubId)) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }

    const guard = await ensureOwnerSubscriptionActive(clubId, access.userId);
