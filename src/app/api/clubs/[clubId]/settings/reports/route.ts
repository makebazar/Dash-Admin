...
export async function POST(
  ...
  try {
    ...
    const validAccountIds = new Set(
      validAccountsRes.rows.map((row: any) => Number(row.id)),
    );
    const sanitizedSchema = normalizedSchema.map((field: any) => {
      ...
      if (
        nextField.account_id === undefined ||
        nextField.account_id === null ||
        nextField.account_id === ""
      ) {
        nextField.account_id = null;
        return nextField;
      }
    ...
    try {
      const incomeFields = sanitizedSchema.filter(
        (f: any) => f.field_type === "INCOME" && f.account_id,
      );
      ...
      for (const field of incomeFields) {
        ...
        if (field.metric_key === "card_income")
          metricKeys.push("card", "terminal");
        ...
        const descriptionMatch = field.custom_label
          ? `%${field.custom_label}%`
          : null;
      ...
      console.error(
        "Error synchronizing finance on mapping change:",
        syncError,
      );
