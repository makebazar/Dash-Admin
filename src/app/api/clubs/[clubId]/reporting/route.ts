...
export async function GET(
  ...
  try {
    ...
    if (rawQuery.query === "revenue") {
      ...
      const { period, startDate, endDate, employeeId, shiftType } =
        validation.data;
      const { startDateSql, endDateSql } = getDateRange(
        period,
        startDate,
        endDate,
      );
      ...
      const { period, startDate, endDate, employeeId, status } =
        validation.data;
      const { startDateSql, endDateSql } = getDateRange(
        period,
        startDate,
      ...
      return NextResponse.json(
        { error: `Unsupported query type: '${rawQuery.query}'` },
        { status: 400 },
      );
    }
  } catch (error: any) {
