...
export async function GET(
  ...
  try {
    ...
    await requireModuleAccess(clubId, "schedule", "view");
    ...
    try {
      ...
      console.error(
        "Work Schedule Auto-migration Failure (Non-fatal):",
        dbError.message,
      );
    }
    ...
    try {
      ...
      console.warn(
        "Failed to fetch club settings (using defaults):",
        e.message,
      );
    }
...
export async function PATCH(
  ...
  try {
    ...
    await requireModuleAccess(clubId, "schedule", "edit");
    ...
    if (shiftType === null) {
      await query(
        `DELETE FROM work_schedules WHERE club_id = $1 AND user_id = $2 AND date = $3`,
        [clubId, userId, date],
      );
      ...
      if (candidateTasks.rowCount && candidateTasks.rowCount > 0) {
        ...
        for (const task of candidateTasks.rows) {
          ...
          if (
            taskDate.getTime() === newShiftDate.getTime() &&
          ...
          if (task.last_cleaned_at) {
            ...
            if (newShiftDate < minDate) canMove = false;
          }
          ...
          if (canMove) {
            ...
            if (!task.assigned_user_id) {
              ...
              if (currentShiftCheck.rowCount === 0) shouldMove = true;
              else if (newShiftDate < taskDate) shouldMove = true;
            }
