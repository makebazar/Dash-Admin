  ...
  try {
    ...
    const employees = (result.rows || [])
      ...
      .map((row) => {
        const isOwner =
          row.club_owner_id &&
          String(row.club_owner_id) === String(row.user_id);
        const isOwnerLike = isOwner || String(row.club_role) === "Владелец";
        const baseRoleName =
          row.club_role === "EMPLOYEE" ||
          row.club_role === "Сотрудник" ||
          row.club_role === "EMP"
            ? row.role_name || "Сотрудник"
            : row.club_role || row.role_name || "Сотрудник";

        const configuredShiftRoles =
          shiftRolesMap.get(String(row.user_id)) || [];
        const fallbackShiftRoles: {
          role_id: number;
          role_name: string;
          priority: number;
        }[] = [];
        if (isOwnerLike && ownerRoleId) {
          fallbackShiftRoles.push({
            role_id: ownerRoleId,
            role_name: "Владелец",
            priority: 0,
          });
        ...
        if (row.role_id) {
          ...
          if (Number.isFinite(rid) && (!ownerRoleId || rid !== ownerRoleId)) {
            fallbackShiftRoles.push({
              role_id: rid,
              role_name: String(row.role_name || baseRoleName || "Сотрудник"),
              priority: fallbackShiftRoles.length,
            });
          }
        }
        ...
        return {
          ...
          shift_roles:
            configuredShiftRoles.length > 0
              ? configuredShiftRoles
              : fallbackShiftRoles,
...
export async function POST(
  ...
  try {
    ...
    if (role_id) {
      const roleRes = await query("SELECT name FROM roles WHERE id = $1", [
        role_id,
      ]);
