...
export async function PUT(
  ...
  try {
    ...
    if (body?.name !== undefined)
      fields.push({ sql: "name", value: String(body.name || "").trim() });
    if (body?.description !== undefined)
      fields.push({ sql: "description", value: body.description ?? null });
    if (body?.position !== undefined)
      fields.push({ sql: "position", value: body.position ?? null });
    if (body?.schema !== undefined)
      fields.push({ sql: "schema", value: body.schema });
    if (body?.is_active !== undefined)
      fields.push({ sql: "is_active", value: Boolean(body.is_active) });
    ...
    if (tests !== undefined) {
      ...
      await query(
        `DELETE FROM recruitment_form_template_tests WHERE template_id = $1`,
        [templateId],
      );
...
export async function DELETE(
  ...
  try {
    ...
    const status = typeof error?.status === "number" ? error.status : 500;
    if (status === 500)
      console.error("Recruitment Template DELETE Error:", error);
    return NextResponse.json(
      { error: error?.message || "Internal Server Error" },
      { status },
    );
