  ...
  try {
      ...
      employeeRoleSchemeMap.set(
        `${String(row.user_id)}||${String(row.role_id)}`,
        {
          ...
          standard_monthly_shifts:
            row.scheme_standard_monthly_shifts ||
            formula.standard_monthly_shifts,
        },
      );
    }
    ...
    const summary = await Promise.all(
      employees.map(async (emp: any) => {
        ...
        const empShifts = shiftsRes.rows.filter(
          (s: any) => s.user_id === emp.id,
        );
        ...
        monthlyMetrics["maintenance_old_debt_closed_tasks"] =
          oldDebtClosedTasks;
        ...
        if (snapshot?.frozen_bonuses_status) {
          ...
          const calculateBonusStatus = (
            ...
            const metricKey =
              bonus.metric_key || bonus.source || "total_revenue";
            const current_value =
            ...
            if (Array.isArray(enrichedBonuses)) {
              enrichedBonuses = enrichedBonuses.map((bonus: any) => {
                if (
                  ...
                  const status = calculateBonusStatus(
                    bonus,
                    schemeCtx,
                    roleMeta,
                  );
        ...
        const processedShifts = await Promise.all(
          empShifts.map(async (s: any) => {
            ...
            const roleMeta = roleIdSnapshot
              ? roleContexts.find(
                  (r) => Number(r.role_id) === roleIdSnapshot,
                ) || fallbackRole
              : fallbackRole;
            ...
            reportMetricsForShift["maintenance_tasks_completed"] =
              monthCompletedTasks;
            reportMetricsForShift["maintenance_tasks_assigned"] =
              monthTotalTasks;
        ...
        const total_paid_bonus = parseFloat(
          empPayment?.total_paid_bonus || "0",
        );
        ...
        if (maintConfigForPayout) {
          ...
          const reworkOpen =
            monthlyMetrics["maintenance_rework_open_tasks"] || 0;
          const staleRework =
            monthlyMetrics["maintenance_stale_rework_tasks"] || 0;
          const overdueOpen =
            monthlyMetrics["maintenance_overdue_open_tasks"] || 0;
          let efficiency =
            monthlyMetrics["maintenance_adjusted_efficiency"] || 0;
          ...
          if (
            ...
            current_thresholds =
              maintConfigForPayout.maintenance_thresholds.map((t: any) => ({
                from: t.min_tasks,
                amount: t.amount,
        ...
        let final_total_accrued =
          base_salary + shift_bonuses + kpi_bonus_amount;
          ...
          display_bonuses.forEach((bonus: any) => {
            if (bonus.metric_key) {
              ...
              revenue_by_metric[bonus.metric_key] = {
                total: metric_total,
                avg_per_shift:
                  shifts_count > 0 ? metric_total / shifts_count : 0,
              };
            }
          });
        }
        ...
        return {
          ...
          metrics: {
            ...
            maintenance_bonus_base:
              monthlyMetrics["maintenance_bonus_base"] || 0,
    ...
    const leaderboardState = await getClubEmployeeLeaderboardState(
      clubId,
      year,
      month,
    );
    ...
    const summaryWithLeaderboard = summary.map((employee: any) => {
      ...
      return {
        ...
        virtual_balance:
          cleanEmployee.virtual_balance + leaderboardBonusVirtual,
        period_bonuses: [
          ...cleanEmployee.period_bonuses,
          ...leaderboardStatuses,
        ],
        has_kpi_feature:
        ...
        breakdown: {
          ...
          kpi_bonuses:
            cleanEmployee.breakdown.kpi_bonuses + leaderboardBonusReal,
          accrued_payout:
