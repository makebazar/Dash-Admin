...
export async function POST(
    ...
    try {
        ...
        if (!equipment_id) {
            return NextResponse.json({ error: 'equipment_id обязателен' }, { status: 400 });
        }
        ...
        const tasksBefore = taskCountResult.rows.reduce((acc: Record<string, number>, row: any) => {
        ...
        const tasksAfter = taskCountAfterResult.rows.reduce((acc: Record<string, number>, row: any) => {
            acc[row.status] = parseInt(row.count);
            return acc;
        }, {});

        const cancelled = (tasksBefore['PENDING'] || 0) + (tasksBefore['IN_PROGRESS'] || 0) - (tasksAfter['PENDING'] || 0) - (tasksAfter['IN_PROGRESS'] || 0);
