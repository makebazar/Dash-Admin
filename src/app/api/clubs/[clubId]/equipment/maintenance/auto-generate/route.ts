...
export async function POST(
    ...
    try {
        ...
        const activeEmployeeIds = new Set(activeEmployeesResult.rows.map((r: any) => r.user_id));
        ...
        const findNextShiftDate = (userId: string, targetDate: string, visited: Set<string>): string | null => {
        ...
        for (const eq of equipmentRows) {
            ...
            let lastCleaned = eq.last_cleaned_at ? new Date(eq.last_cleaned_at) : null;
            ...
            if (autoAssign) {
                ...
                if (!assignedUserId) {
                    for (const empId of activeEmployeeIds) {
                        const shiftDate = findNextShiftDate(empId, dueDateStr, new Set<string>());
                        if (shiftDate) {
