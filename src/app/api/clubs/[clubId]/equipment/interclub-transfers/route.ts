...
export async function POST(
    ...
    if (!targetClubId || typeof targetClubId !== 'number') {
        return NextResponse.json({ error: 'target_club_id is required' }, { status: 400 })
    }

    if (String(targetClubId) === String(clubId)) {
        return NextResponse.json({ error: 'Нельзя перемещать в тот же клуб' }, { status: 400 })
    }

    const uniqueItems = Array.from(
        new Map(
            itemsRaw
                .filter((i: any) => i && typeof i.equipment_id === 'string' && i.equipment_id.trim())
                .map((i: any) => [
                    i.equipment_id.trim(),
                    {
                        equipment_id: i.equipment_id.trim(),
                        target_workstation_id:
                            typeof i.target_workstation_id === 'string' && i.target_workstation_id.trim()
    ...
    if (uniqueItems.length > 50) {
        return NextResponse.json({ error: 'Слишком много позиций (макс 50)' }, { status: 400 })
    }

    const createAccess = await query(
        `SELECT 1 FROM clubs WHERE id = $1 AND owner_id = $2
         UNION
         SELECT 1
         FROM club_employees
         WHERE club_id = $1 AND user_id = $2 AND is_active = TRUE
           AND role IN ('Админ', 'Управляющий')`,
        [clubId, userId]
    )
    if ((createAccess.rowCount || 0) === 0) {
        return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }
    ...
    if ((targetClubCheck.rowCount || 0) === 0) {
        return NextResponse.json({ error: 'Недоступный клуб назначения' }, { status: 400 })
    }
    ...
    try {
        ...
        const sourceClubNameRes = await client.query(`SELECT name FROM clubs WHERE id = $1`, [clubId])
        const sourceClubName = sourceClubNameRes.rows[0]?.name || `Клуб ${clubId}`

        const equipmentIds = uniqueItems.map((i) => i.equipment_id)
        ...
        const workstationIds = Array.from(
            new Set(uniqueItems.map((i) => i.target_workstation_id).filter(Boolean).map((v) => String(v)))
        )
        ...
        for (const item of uniqueItems) {
            const equipment = equipmentRes.rows.find((r: any) => String(r.id) === String(item.equipment_id))
            ...
            await client.query(
                ...
                [
                    ...
                    item.target_workstation_id ? String(item.target_workstation_id) : null,
                ]
            )
            ...
            const resolvedState = resolveEquipmentStateForPersistence({
                ...
                currentWorkstationId: equipment.workstation_id ? String(equipment.workstation_id) : null,
                ...
                await client.query(
                    `UPDATE equipment
                     SET workstation_id = $1,
                         ...
                         status = $3
                     WHERE id = $4`,
                    [resolvedState.workstation_id, resolvedState.is_active, resolvedState.status, item.equipment_id]
                )
            } else {
                await client.query(
                    `UPDATE equipment
                     SET workstation_id = $1,
                         ...
                         is_active = $2
                     WHERE id = $3`,
                    [resolvedState.workstation_id, resolvedState.is_active, item.equipment_id]
                )
            }
        }
        ...
        if (status >= 500) console.error('Create Interclub Transfer Error:', error)
