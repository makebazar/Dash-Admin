  ...
  try {
    ...
    let sqlQuery = `
            SELECT
                ...
                s.report_data,
                s.report_mode,
                s.actor_role_name_snapshot,
                s.actor_role_id_snapshot,
...
export async function POST(
  ...
  try {
    ...
    if ((employeeCheck.rowCount || 0) === 0) {
      return NextResponse.json(
        { error: "Employee not found in this club" },
        { status: 400 },
      );
    }
    ...
    if ((schemeRes.rowCount || 0) > 0) {
      ...
      const calculation = await calculateSalary(
        ...
        {
          ...(scheme.formula || {}),
          standard_monthly_shifts: scheme.standard_monthly_shifts,
        },
        {
          total_revenue:
            (Number(cash_income) || 0) + (Number(card_income) || 0),
