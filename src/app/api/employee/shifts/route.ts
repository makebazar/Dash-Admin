  ...
  try {
    ...
    if ((accessCheck.rowCount || 0) === 0) {
      return NextResponse.json(
        { error: "You are not an employee of this club" },
        { status: 403 },
      );
    }
    ...
    if ((activeCheck.rowCount || 0) > 0) {
      return NextResponse.json(
        { error: "You already have an active shift in this club" },
        { status: 400 },
      );
    }
    ...
    const hasShiftRoleNameSnapshot = await hasColumn(
      "shifts",
    ...
    const assignedRoleIds = assignedRolesRes.rows
    ...
    const requestedRoleId =
      role_id === null || role_id === undefined ? null : Number(role_id);
    ...
    if (isOwner) {
      ...
      const ownerRoleRes = await query(
        `SELECT id FROM roles WHERE name = 'Владелец' LIMIT 1`,
      );
      ownerRoleId = ownerRoleRes.rows[0]?.id
        ? Number(ownerRoleRes.rows[0].id)
        : null;
    }

    let fallbackRoleId: number | null = null;
    if (assignedRoleIds.length === 0) {
      const fallbackRoleRes = await query(
        `SELECT role_id FROM users WHERE id = $1`,
        [userId],
      );
      fallbackRoleId = fallbackRoleRes.rows[0]?.role_id
        ? Number(fallbackRoleRes.rows[0].role_id)
        : null;
    }
    ...
    if (effectiveRoleId !== null) {
      const roleNameRes = await query(`SELECT name FROM roles WHERE id = $1`, [
        effectiveRoleId,
      ]);
      effectiveRoleName = roleNameRes.rows[0]?.name
        ? String(roleNameRes.rows[0].name)
        : null;
    }
    ...
    if (!isNaN(hourFloat)) {
      if (dayStartHour < nightStartHour) {
        ...
        if (
          hourFloat >= dayStartWithTolerance &&
          hourFloat < nightStartWithTolerance
        ) {
          shiftType = "DAY";
        } else {
          shiftType = "NIGHT";
