...
function getFileExtension(fileName: string) {
  const match = sanitizeFileName(fileName)
    .toLowerCase()
    .match(/\.([a-z0-9]+)$/);
  return match?.[1] || "";
}
...
function getNormalizedMimeType(fileName: string, mimeType: string) {
  ...
  return (
    MIME_TYPE_BY_EXTENSION[getFileExtension(fileName)] ||
...
async function transcodeVideoToWebm(
  buffer: Buffer | Uint8Array,
  fileName: string,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const preserveOriginal =
      String(formData.get("preserveOriginal") || "").trim() === "1";
    const transcodeVideo =
      String(formData.get("transcodeVideo") || "").trim() === "1";
    ...
    const finalUpload = shouldTranscodeVideo(
    ...
    const url = await uploadFileToS3(
      finalUpload.buffer,
      finalUpload.fileName,
      finalUpload.mimeType,
    );
