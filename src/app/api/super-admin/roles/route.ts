...
export async function GET() {
  try {
    ...
    const hasEmployeeAccessSettings = await hasColumn(
      "roles",
...
export async function POST(request: Request) {
  try {
    ...
    if (name.length > 50)
      return NextResponse.json(
        { error: "Название роли слишком длинное (макс 50)" },
        { status: 400 },
      );
  ...
  try {
    ...
    if (!hasEmployeeAccessSettings) {
      return NextResponse.json(
        {
          error:
            "Колонка employee_access_settings отсутствует. Примените миграции.",
        },
        { status: 400 },
      );
    }
    ...
    const isCustomizable = body?.is_customizable;

    if (!Number.isFinite(roleIdNum))
      return NextResponse.json({ error: "roleId обязателен" }, { status: 400 });

    let updateQuery = "";
    let updateParams: any[] = [];
    let returningCols = "id, name, employee_access_settings, is_customizable";

    if (employeeAccessSettings !== undefined && isCustomizable !== undefined) {
      if (
        typeof employeeAccessSettings !== "object" ||
        employeeAccessSettings === null
      ) {
        return NextResponse.json(
          { error: "employee_access_settings должен быть объектом" },
          { status: 400 },
        );
      }
      updateQuery = `UPDATE roles SET employee_access_settings = $1::jsonb, is_customizable = $2 WHERE id = $3 RETURNING ${returningCols}`;
      updateParams = [
        JSON.stringify(employeeAccessSettings),
        Boolean(isCustomizable),
        roleIdNum,
      ];
    } else if (employeeAccessSettings !== undefined) {
      if (
        typeof employeeAccessSettings !== "object" ||
        employeeAccessSettings === null
      ) {
      ...
      updateQuery = `UPDATE roles SET employee_access_settings = $1::jsonb WHERE id = $2 RETURNING ${returningCols}`;
      updateParams = [JSON.stringify(employeeAccessSettings), roleIdNum];
    } else if (isCustomizable !== undefined) {
      updateQuery = `UPDATE roles SET is_customizable = $1 WHERE id = $2 RETURNING ${returningCols}`;
      updateParams = [Boolean(isCustomizable), roleIdNum];
    } else {
      return NextResponse.json(
    ...
    const update = await query(updateQuery, updateParams);
    ...
    return NextResponse.json({
      role: {
        ...
        is_customizable: Boolean(update.rows[0].is_customizable),
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (reassignRoleIdNum !== null && !Number.isFinite(reassignRoleIdNum)) {
      return NextResponse.json(
        { error: "reassignRoleId должен быть числом" },
        { status: 400 },
      );
    }
    ...
    if (reassignRoleIdNum !== null) {
      if (reassignRoleIdNum === roleIdNum) {
        return NextResponse.json(
          { error: "Нельзя переназначить на ту же роль" },
          { status: 400 },
        );
      }
      ...
      if ((toRes.rowCount || 0) === 0)
        return NextResponse.json(
          { error: "Роль для переназначения не найдена" },
          { status: 400 },
        );
