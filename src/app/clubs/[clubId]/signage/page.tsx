...
export default function ClubSignagePage({
  ...
  const [previewDevice, setPreviewDevice] = useState<SignageDevice | null>(
    null,
  );
  const [previewLayout, setPreviewLayout] = useState<SignageLayout | null>(
    null,
  );
  ...
  const [unpairingDeviceId, setUnpairingDeviceId] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const init = async () => {
      const { clubId: nextClubId } = await params;
      if (!isMounted) return;
  ...
  async function fetchDevices(targetClubId = clubId) {
    ...
    try {
      const res = await fetch(`/api/clubs/${targetClubId}/signage/devices`, {
        cache: "no-store",
      });
  ...
  async function handleOpenPreview(device: SignageDevice) {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/signage/devices/${device.id}/layout`,
        {
          cache: "no-store",
        },
      );
  ...
  async function handleRenameDevice() {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/signage/devices/${renameDevice.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  async function handleUnpairDevice() {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/signage/devices/${unpairDevice.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <PageShell maxWidth="5xl">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 rounded-3xl border border-slate-200 bg-white py-24 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900">
              Нет подключенных экранов
            </h3>
            <p className="text-sm text-slate-500 max-w-sm mx-auto">
              Откройте приложение плеера на устройстве и введите код привязки
              ниже, чтобы добавить первый экран.
            </p>
            <Button
              variant="outline"
              ...
              onClick={() =>
                document
                  .getElementById("pairing-section")
                    ...
                    >
                      ...
                      <DropdownMenuItem
                        onClick={() => {
                          ...
                          setRenameValue(
                            device.name || device.screen_label || "",
                          );
                        }}
                        className="gap-3 cursor-pointer rounded-lg px-3 py-2 text-[15px]"
                      >
                ...
                <div className="mb-4 space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-500">
                    ...
                    <span className="truncate text-[13px]">
                      {device.screen_label || "Монитор не выбран"}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <div
        id="pairing-section"
        className="rounded-3xl bg-white border border-slate-200 p-8 sm:p-12 mb-12"
      >
        <div className="mx-auto max-w-2xl text-center space-y-8">
          <div className="space-y-3">
            ...
            <p className="text-slate-500 text-sm">
              Запустите приложение Signage Player на целевом устройстве. На
              экране появится уникальный код — введите его ниже.
            </p>
          </div>
                ...
                <div className="mx-auto max-w-180">
                  <div
                    className={cn(
                      ...
                      previewDevice.orientation === "portrait"
                        ? "w-60"
                        : "w-full",
                    )}
                  >
      ...
      <Dialog
        open={Boolean(unpairDevice)}
        onOpenChange={(open) => !open && setUnpairDevice(null)}
      >
        <DialogContent className="sm:max-w-100 rounded-3xl border border-slate-200 shadow-2xl bg-white p-6">
          <DialogHeader className="space-y-3">
            ...
            <div>
              ...
              <DialogDescription className="text-sm font-medium text-slate-500">
                Вы собираетесь отвязать экран{" "}
                <strong>
                  {unpairDevice?.name ||
                    unpairDevice?.screen_label ||
                    "Без названия"}
                </strong>{" "}
                от клуба
              </DialogDescription>
          ...
          <div className="py-4">
            <div className="bg-rose-50 border border-rose-100 p-4 rounded-xl space-y-2">
              <p className="text-[11px] text-rose-700 leading-relaxed font-medium">
                После отвязки этот экран перестанет получать расписание
                трансляций и перейдет в режим ожидания привязки.
              </p>
            </div>
          </div>
