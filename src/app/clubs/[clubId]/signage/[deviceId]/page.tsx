...
const SLIDE_TRANSITION_OPTIONS: SignageTransition[] = [
  "none",
  "fade",
  "slide",
  "zoom",
...
function getTimeRangeSummary(slide: SignageSlide) {
  if (
    slide.startHour === slide.endHour &&
    slide.startMinute === slide.endMinute
  ) {
    return "24 часа";
  }
...
function getDeviceContentSize(device: SignageDevice | null) {
  if (
    !device ||
    !Array.isArray(device.display_info) ||
    device.display_info.length === 0
  ) {
    return null;
  }

  const selectedDisplay =
    device.display_info.find(
      (display) =>
        String(display.id || "") === String(device.selected_display_id || ""),
    ) ||
...
export default function ClubSignageDeviceEditorPage({
  ...
  async function fetchEditorData(
    ...
    try {
      const [devicesRes, layoutRes] = await Promise.all([
        fetch(`/api/clubs/${targetClubId}/signage/devices`, {
          cache: "no-store",
        }),
        fetch(
          `/api/clubs/${targetClubId}/signage/devices/${targetDeviceId}/layout`,
          {
            cache: "no-store",
          },
        ),
      ...
      const matchedDevice = Array.isArray(devicesData.devices)
        ? devicesData.devices.find(
            (item: SignageDevice) => String(item.id) === String(targetDeviceId),
          )
        : null;

      setDevice(matchedDevice || null);
      setLayoutDraft(
        normalizeSignageLayout(layoutData.layout, layoutData.orientation),
      );
    } catch (error: any) {
      console.error(error);
      showNotice(
        "error",
        error?.message || "Не удалось загрузить редактор экрана",
      );
      setDevice(null);
      setLayoutDraft(null);
    } finally {
  ...
  async function handleSaveLayout() {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/signage/devices/${deviceId}/layout`,
        {
          method: "PATCH",
  ...
  function addWebsiteSlide(url: string, title = "Сайт") {
    const newSlide = createSignageSlide({
      imageUrl: url,
      mediaType: "website",
      title,
      transition: layoutDraft?.transition ?? "fade",
      order: layoutDraft?.slides.length || 0,
      durationSec: 15,
      startHour: 0,
      endHour: 0,
      startMinute: 0,
    ...
    updateLayout((layout) => ({
      ...layout,
      slides: [...layout.slides, newSlide].map((slide, index) => ({
        ...slide,
        order: index,
      })),
    }));
  ...
  function handleAddPromoBoard() {
    const url = `${window.location.origin}/promo/board?clubId=${clubId}`;
    addWebsiteSlide(url, "Promo Board");
  }

  function handleAddCustomUrl() {
    const url = window.prompt(
      "Введите URL (обязательно с http:// или https://)",
    );
    if (url && (url.startsWith("http://") || url.startsWith("https://"))) {
      addWebsiteSlide(url, "Новый сайт");
    } else if (url) {
  ...
  async function handleOrientationChange(
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/signage/devices/${deviceId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  async function handleMediaUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (!mediaType) {
          throw new Error(
            `Файл "${file.name}" не является изображением или видео`,
          );
        }
        ...
        uploadedSlides.push(
          createSignageSlide({
            ...
            transition: layoutDraft?.transition ?? "fade",
            order: layoutDraft?.slides.length
              ? layoutDraft.slides.length + uploadedSlides.length
  ...
  function toggleSlideWeekday(slideId: string, weekday: number) {
    const targetSlide = layoutDraft?.slides.find(
      (slide) => slide.id === slideId,
    );
    if (!targetSlide) return;

    const currentWeekdays =
      Array.isArray(targetSlide.weekdays) && targetSlide.weekdays.length > 0
  ...
  return (
    <div className="flex h-dvh w-full flex-col bg-white overflow-hidden text-slate-900 font-sans">
      ...
      <main className="flex flex-1 overflow-hidden">
        <section className="mx-auto flex h-full w-full max-w-245 flex-col border-x border-slate-200 bg-slate-50/50">
            ...
            <>
              ...
              <div className="flex flex-col gap-3 border-b border-slate-200 p-4">
                <div className="grid grid-cols-2 gap-3">
                  <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 bg-white px-4 py-2.5 text-xs font-medium text-slate-600 transition hover:border-slate-400 hover:bg-slate-50">
                    {isUploading ? (
                  ...
                  <button
                    type="button"
                    onClick={handleAddPromoBoard}
                    className="flex items-center justify-center gap-2 rounded-lg border border-orange-200 bg-orange-50/50 px-4 py-2.5 text-xs font-black uppercase tracking-widest text-orange-600 transition hover:bg-orange-100/50"
                  >
                    <ZapIcon className="h-4 w-4 fill-orange-500" />
                    Promo Board
                  </button>
                </div>
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-600 transition hover:bg-slate-50"
                >
                  <Globe className="h-3.5 w-3.5" />
                  Добавить сайт по ссылке
                </button>
                        ...
                        >
                          <div className="flex gap-3">
                            <div className="flex w-28 shrink-0 flex-col gap-2">
                              <div className="relative flex min-h-33 flex-1 items-center justify-center overflow-hidden rounded-xl bg-slate-950 ring-1 ring-black/5">
                                {slide.mediaType === "video" ? (
                                  ...
                                  />
                                ) : slide.mediaType === "website" ? (
                                  <div className="flex flex-col items-center gap-2 text-white/20">
                                    <Globe className="h-10 w-10" />
                                    <span className="text-[8px] font-black uppercase tracking-widest text-center px-2 line-clamp-2">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  ...
                                  onClick={() => moveSlide(slide.id, 1)}
                                  disabled={
                                    index === layoutDraft.slides.length - 1
                                  }
                                  className="flex h-8 flex-1 items-center justify-center rounded-lg border border-slate-200 text-slate-400 transition hover:border-slate-300 hover:text-slate-700 disabled:opacity-30"
                                >
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px] font-medium text-slate-500">
                                    ...
                                    <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                                      {slide.mediaType === "video"
                                        ? "Видео до конца файла"
                                        : `Показ ${slide.durationSec} сек`}
                                    </span>
                                    <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                              ...
                              <div className="flex flex-wrap items-start gap-2">
                                  ...
                                  {slide.mediaType === "video" ? (
                                    <span
                                      className={cn(
                                        ...
                                        index === 1
                                          ? "block text-center leading-6.25"
                                          : "",
                                      )}
                                    >
                                      Авто
                                    ...
                                    <>
                                      <Input
                                        ...
                                        className="h-8 w-12.5 rounded-lg border-slate-200 bg-white px-2 text-center text-xs font-semibold text-slate-900 focus-visible:ring-0"
                                      />
                                ...
                                <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/80 px-2.5 py-2">
                                  ...
                                  <TimeTextInput
                                    ...
                                    onCommit={(value) =>
                                      updateSlideTime(slide.id, "start", value)
                                    }
                                    className="h-8 w-17.5 rounded-lg border-slate-200 bg-white px-2 text-center font-mono text-xs font-semibold text-slate-900 focus-visible:ring-0"
                                  />
                                  ...
                                  <TimeTextInput
                                    ...
                                    onCommit={(value) =>
                                      updateSlideTime(slide.id, "end", value)
                                    }
                                    className="h-8 w-19.75 rounded-lg border-slate-200 bg-white px-2 text-center font-mono text-xs font-semibold text-slate-900 focus-visible:ring-0"
                                  />
                                </div>

                                <div className="flex h-11.25 items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50/80 px-2 py-2">
                                  <span className="px-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">
                                    Переход
                                  </span>
                                  <div className="flex flex-wrap gap-1 rounded-lg border border-slate-200 bg-white p-1">
                                    {SLIDE_TRANSITION_OPTIONS.map(
                                      (transition) => (
                                        <button
                                          key={transition}
                                          type="button"
                                ...
                                >
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {WEEKDAY_OPTIONS.map((weekday) => {
                                      const isActive = slide.weekdays.includes(
                                        weekday.value,
                                      );
                                      return (
                                        <button
