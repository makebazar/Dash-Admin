...
export function ActiveInventory({
  ...
  const [allProducts, setAllProducts] = useState<
    { id: number; name: string; selling_price?: number; cost_price?: number }[]
  >([]);
  ...
  const handleSaveCorrection = async (productId: number) => {
    ...
    startTransition(async () => {
      try {
        const res = await correctInventoryItem(
          inventoryId,
          productId,
          val,
          clubId,
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (inv.shift_id && inv.created_by) {
          const receipts = await getInventoryShiftReceipts(
            clubId,
            inventoryId,
            { includeVoided: true },
          );
  ...
  const persistCountedItems = useCallback(async () => {
    ...
    await Promise.all(
      itemsToUpdate.map((item) =>
        updateInventoryItem(item.id, item.actual_stock, clubId),
      ),
    );
  }, [items, clubId]);
  ...
  const usesPosRevenue = Boolean(
    inventory?.shift_id && inventory?.sales_capture_mode === "SHIFT",
  );
  const canAddUnaccountedSales = Boolean(
    inventory?.target_metric_key && !usesPosRevenue,
  );
  const canCorrectClosedInventory = Boolean(
    isOwner &&
    isClosed &&
    !inventory?.shift_id &&
    !inventory?.target_metric_key,
  );
  const showInventoryRevenueColumn = Boolean(
    inventory?.target_metric_key && !usesPosRevenue,
  );
  ...
  const summaryCards = [
    {
      ...
      hint: isReadOnly
        ...
        : `${inventorySummary.progress}% заполнено`,
      icon: Package,
    ...
    },
    {
      ...
      hint:
        inventorySummary.shortageValue > 0
          ? `${inventorySummary.shortageValue.toLocaleString("ru-RU")} ₽ по продаже`
          : "Пока не обнаружены",
      icon: ReceiptText,
      tone:
        inventorySummary.shortageItems > 0
    ...
    },
    {
      ...
      hint:
        inventorySummary.excessItems > 0
          ? "Есть найденные позиции сверх ожидания"
          : "Пока не обнаружены",
      icon: Boxes,
      tone:
        inventorySummary.excessItems > 0
  ...
  const handleCloseInventory = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        showMessage({
          title: "Ошибка",
          description: e?.message || "Ошибка при закрытии инвентаризации",
        });
      } finally {
        setIsSaving(false);
      }
  ...
  const addUnaccountedSale = () => {
    ...
    if (inInventory) {
      showMessage({
        title: "Товар уже в инвентаризации",
        description:
          "Укажите остаток по этому товару в основном списке, а не как неучтенную продажу.",
      });
  ...
  const handleAddProduct = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        showMessage({
          title: "Ошибка",
          description: e.message || "Ошибка при добавлении товара",
        });
      }
    });
  };
        ...
        setItems((prev) =>
          prev.map((i) =>
            i.id === existingItem.id
              ? { ...i, actual_stock: newStock, last_modified: Date.now() }
              : i,
          ),
        );
      ...
      try {
        ...
        if (product) {
          ...
          if (confirmed) {
            ...
            const updatedItems = invItems.map((i) =>
              i.product_id === product.id
                ? { ...i, actual_stock: 1, last_modified: Date.now() }
                : i,
            );
          ...
          showMessage({
            ...
            description: `Товар со штрихкодом ${barcode} не найден в базе.`,
          });
        }
      } catch (e) {
        console.error(e);
  ...
  const compareInventoryItems = useCallback(
    (a: InventoryItem, b: InventoryItem, searchTerm?: string) => {
  ...
  const groupedItems = useMemo(() => {
    ...
    if (quickFilter !== "all") {
      filtered = filtered.filter((item) => {
        ...
        if (quickFilter === "sold")
          return soldProductIds.has(Number(item.product_id));
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <Button
                aria-label="Вернуться к списку инвентаризаций"
                variant="outline"
                size="icon"
                onClick={onClose}
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                  <span>
                    Начата:{" "}
                    {new Date(inventory.started_at).toLocaleString("ru-RU")}
                  </span>
                ...
                {!isReadOnly && (
                  <div className="rounded-2xl border bg-card/80 p-3">
                    ...
                    <p className="mt-2 text-xs text-muted-foreground">
                      {uncountedCount === 0
                        ? "Можно завершать подсчёт — все позиции заполнены."
                        : `Осталось проверить ${uncountedCount} поз.`}
                    </p>
                  </div>
                )}
              </div>
            ...
            {!isReadOnly && (
              <div className="grid gap-2 sm:grid-cols-3 xl:grid-cols-1">
                <Button
                  ...
                  className="h-11 rounded-xl bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100"
                >
        ...
        <CardHeader>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2 text-xs text-muted-foreground">
            <Badge variant="outline" className="bg-card">
              Групп по категориям: {groupedItems.length}
            </Badge>
            <Badge variant="outline" className="bg-card">
            ...
            <Table>
              ...
              <TableBody>
                {groupedItems.map(([category, categoryItems]) => (
                  <React.Fragment key={`cat-group-${category}`}>
                    ...
                    {categoryItems.map((item) => {
                      ...
                      const difference =
                        (item.actual_stock || 0) - (item.expected_stock || 0);
                      const inventoryValue =
                        difference * item.selling_price_snapshot;
                            ...
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2">
                                ...
                                {!isReadOnly && item.actual_stock !== null && (
                                  <Badge
                                    variant="outline"
                                    className="h-5 border-blue-200 bg-blue-50 text-[10px] font-bold text-blue-700"
                                  >
                                    Посчитан
                                  </Badge>
                                )}
                                {soldProductIds.has(
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="h-5 border-violet-200 bg-violet-50 text-[10px] font-bold text-violet-700"
                                  >
                                    Касса ·{" "}
                                    {posSoldMap.get(Number(item.product_id)) ||
                                      0}{" "}
                                    шт
                                  </Badge>
                                )}
                              ...
                              {(item.barcode ||
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                  ...
                                  {item.barcodes?.map((bc: string) => (
                                    <span
                                      key={bc}
                                      className="text-[10px] text-muted-foreground font-mono flex items-center gap-1 bg-accent px-1.5 py-0.5 rounded"
                                    >
                                      <Barcode className="h-2.5 w-2.5" />
                                      {bc}
                                    </span>
                              ...
                              editingItemId === item.id ? (
                                <div className="flex items-center justify-end gap-1">
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      setCorrectionStock(e.target.value)
                                    }
                                  />
                                  <Button
                                    ...
                                    onClick={() =>
                                      handleSaveCorrection(item.product_id)
                                    }
                                    disabled={isPending}
                                  >
                ...
                {items.length === 0 && (
                  <TableRow>
                    <TableCell
                      colSpan={isClosed ? 6 : isReadOnly || isOwner ? 4 : 3}
                          ...
                          <div className="flex flex-wrap gap-1">
                            ...
                            {soldProductIds.has(Number(item.product_id)) && (
                              <span className="text-[9px] font-bold uppercase tracking-wider text-violet-700 bg-violet-50 px-1 rounded border border-violet-200">
                                Касса ·{" "}
                                {posSoldMap.get(Number(item.product_id)) || 0}{" "}
                                шт
                              </span>
                            )}
                        ...
                        <div className="flex flex-col items-end gap-1">
                          {isReadOnly ? (
                            <div className="flex flex-col items-end">
                              <div className="flex items-center gap-2">
                                <span className="text-xs text-muted-foreground/70">
                                  Было: {item.expected_stock}
                                </span>
                                <span className="text-sm font-black text-foreground">
                                  Стало: {item.actual_stock}
                                </span>
                              </div>
                            ...
                            <div className="flex items-center gap-3">
                              {(isReadOnly || isOwner) && (
                                <div className="flex flex-col items-end">
                                  <span className="text-[9px] text-muted-foreground/70 uppercase font-bold tracking-widest">
                                    Ожидалось
                                  </span>
      ...
      {isClosed && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Показывает все изменения, которые были внесены после закрытия
              ревизии.
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            ...
                            className={cn(
                              "text-[10px] font-bold",
                              correction.stock_delta > 0
                                ? "border-green-200 bg-green-50 text-green-700"
                                : "border-red-200 bg-red-50 text-red-700",
                            )}
                          >
                            {correction.stock_delta > 0
                              ? `+${correction.stock_delta}`
                              : correction.stock_delta}{" "}
                            шт
                          </Badge>
                        </div>
                        <p className="text-xs text-muted-foreground">
                          Было: {correction.old_actual_stock} · Стало:{" "}
                          {correction.new_actual_stock} · Разница до:{" "}
                          {correction.difference_before ?? "—"} · После:{" "}
                          {correction.difference_after}
                        </p>
                        <p className="text-xs text-muted-foreground">
                      ...
                      <div className="text-xs text-muted-foreground md:text-right">
                        ...
                        <div className="font-semibold text-foreground">
                          {correction.created_by_name || correction.created_by}
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Выберите товар, который был найден на складе, но отсутствует в
              списке.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-125">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl border bg-muted p-3">
                  ...
                  <p className="mt-1 text-lg font-black text-foreground">
                    {inventorySummary.shortageItems}
                  </p>
                </div>
                <div className="rounded-2xl border bg-muted p-3">
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">
                            ...
                            <TableCell className="py-1">
                              <div className="flex items-center gap-1">
                                {s.isUnaccounted && (
                                  <Badge
                                    variant="outline"
                                    className="text-[9px] h-4 px-1 bg-blue-50 text-blue-600 border-blue-100"
                                  >
                                    Неучт.
                                  </Badge>
                                )}
                  ...
                  {unaccountedSales.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {unaccountedSales.map((sale) => (
                        <div
                          key={sale.product_id}
                          className="flex items-center gap-1 bg-accent px-2 py-1 rounded-full text-[10px] border border-border"
                        >
                          <span>
                            {sale.name} ({sale.quantity} шт)
                          </span>
                          <button
                            aria-label={`Удалить неучтенную продажу ${sale.name}`}
                            onClick={() =>
                              removeUnaccountedSale(sale.product_id)
                            }
          ...
          {!inventory.target_metric_key && (
            <div className="space-y-3 py-4">
              ...
              <p className="text-sm text-muted-foreground">
                Остатки на складе будут обновлены по введённым фактическим
                значениям. Перед подтверждением проверьте недостачи и излишки.
              </p>
            </div>
          )}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                (!!inventory.target_metric_key && !reportedRevenue) ||
                isPending ||
                isSaving
              }
