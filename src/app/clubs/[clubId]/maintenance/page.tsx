    ...
    const handleAssign = async (taskId: string, userId: string) => {
        try {
            const res = await fetch(`/api/clubs/${clubId}/maintenance/tasks/${taskId}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ assigned_user_id: userId === '' ? null : userId })
            })
            if (res.ok) {
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="flex items-end gap-3">
                                    ...
                                    <span className="text-muted-foreground font-medium pb-1.5">{completedCount} из {tasks.length} ПК обслужено</span>
                                </div>
                            </div>
                            ...
                            {tasks.length === 0 ? (
                                ...
                                <>
                                    <div className="bg-background/80 backdrop-blur-sm p-4 rounded-2xl border shadow-sm text-center min-w-[140px]">
                                        ...
                                        <p className="text-3xl font-black">{tasks.length - completedCount}</p>
                                    </div>
                                    <div className="bg-background/80 backdrop-blur-sm p-4 rounded-2xl border shadow-sm text-center min-w-[140px]">
                                        <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mb-1">Без админа</p>
                                        <p className="text-3xl font-black text-amber-500">{tasks.filter(t => !t.assigned_user_id).length}</p>
                                    </div>
                                </>
                            )}
                        </div>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                    ...
                    <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1.5 ml-4"><div className="h-2 w-2 rounded-full bg-amber-500 animate-pulse"></div> В работе</div>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-foreground/5">
                                {tasks.map(task => (
                                    <tr key={task.id} className="hover:bg-muted/40 transition-colors group">
                                        <td className="p-5">
                                            <div className="flex items-center gap-4">
                                                <div className={`p-2.5 rounded-xl transition-all ${task.status === 'COMPLETED' ? 'bg-green-100 text-green-600' : 'bg-primary/5 text-primary'}`}>
                                                    <Monitor className="h-5 w-5" />
                                                </div>
                                                <div>
                                                    <p className="font-black text-base">{task.workstation_name}</p>
                                                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tighter">Workstation ID: {task.workstation_id.slice(0, 8)}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-5">
                                        ...
                                        <td className="p-5">
                                            {task.status === 'COMPLETED' ? (
                                                <div className="flex items-center gap-1.5 text-green-600 font-bold px-3 py-1 bg-green-50 rounded-full w-fit border border-green-100">
                                                    <CheckCircle2 className="h-4 w-4" /> <span className="text-[11px] uppercase tracking-wider">Завершено</span>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-1.5 text-amber-600 font-bold px-3 py-1 bg-amber-50 rounded-full w-fit border border-amber-100">
                                                    <Clock className="h-4 w-4 animate-spin-pulse" /> <span className="text-[11px] uppercase tracking-wider">В процессе</span>
                                                </div>
                                            )}
                                        </td>
                                        <td className="p-5 text-right">
                                            {task.completed_at ? (
                                                <div className="flex flex-col items-end">
                                                    <span className="font-black text-foreground">{new Date(task.completed_at).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span>
                                                    <span className="text-[10px] text-muted-foreground font-medium">{new Date(task.completed_at).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</span>
                                                </div>
                                            ) : (
