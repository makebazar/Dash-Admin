...
async function getDashboardAccess(
  ...
  try {
    ...
    // For Dashboard page, we also need permissions map for UI elements
...
async function getShiftStats(clubId: string): Promise<ShiftStats> {
  ...
  const expensesTotal = currentRows.reduce(
    (sum: number, row: any) => sum + normalizeMetricValue(row.expenses),
    0,
  );
  ...
  const previousExpenses = previousRows.reduce(
    (sum: number, row: any) => sum + normalizeMetricValue(row.expenses),
    0,
  );
  ...
  return {
    ...
    expensesChange:
      previousExpenses > 0
        ? ((expensesTotal - previousExpenses) / previousExpenses) * 100
        : 0,
    operatingResult,
    operatingChange:
...
async function getRevenueTrend(
  ...
  rows.forEach((row: any) => {
    ...
    revenueByDate.set(
      key,
      current + calculateShiftIncomeTotal(row, metricMeta),
    );
  });
...
async function getRevenueInsights(
  ...
  if (weekendAvg > 0 || weekdayAvg > 0)
    insights.push({
      ...
      description:
        weekendDeltaPercent >= 0
          ? `Выходные сильнее. В среднем ${formatCurrency(weekendAvg)} против ${formatCurrency(weekdayAvg)}`
          : `Будни сильнее. В среднем ${formatCurrency(weekdayAvg)} против ${formatCurrency(weekendAvg)}`,
  ...
  if (bestShiftCombo)
    ...
    insights.push({
      ...
      description:
        latestVsTypical >= 0
          ? `Выше обычного на ${formatSignedPercent(latestVsTypical)}`
          : `Ниже обычного на ${formatSignedPercent(latestVsTypical)}`,
...
async function getNextScheduledShift(
  clubId: string,
...
async function getInventorySnapshot(
  ...
  const [
    ...
    sales,
    criticalItemsResult,
    categoryAMetricsResult,
    categoryAItemsResult,
  ] = await Promise.all([
    getClubTasks(clubId),
  ...
  const recentSales = sales.filter(
    (item: any) => new Date(item.created_at) >= since,
  );
  ...
  recentSales.forEach((item: any) => {
    ...
    uniqueShiftDiscrepancies.set(
      shiftId,
      Math.abs(Number(item.shift_revenue_difference || 0)),
    );
  });
  ...
  const categoryAItems = categoryAItemsResult.rows.map((row: any) => {
    ...
    const status =
      currentStock <= 0 || (minStockLevel > 0 && currentStock < minStockLevel)
        ? "critical"
  ...
  return {
    ...
    returnAmount: returnRows.reduce(
      (sum: number, item: any) =>
        sum +
        Math.abs(
          Number(item.price_at_time || 0) * Number(item.change_amount || 0),
        ),
      0,
    ),
    returnCount: returnRows.length,
    discrepancyAmount: Array.from(uniqueShiftDiscrepancies.values()).reduce(
      (sum, value) => sum + value,
      0,
    ),
        ...
        <section>
          <div className="flex items-baseline justify-between mb-6">
            ...
            <Link
              ...
              className="group flex items-center text-sm font-medium text-slate-500 hover:text-black transition-colors"
            >
              Все смены{" "}
                ...
                {activeShifts.length > 0 ? (
                  <div className="space-y-3">
                    {activeShifts.slice(0, 3).map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between bg-white rounded-xl border border-slate-200 p-4"
                      >
                        <div>
              ...
              {inventorySnapshot.categoryAItems.length > 0 ? (
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-slate-500">
                          {item.daysLeft !== null
                            ? `запас: ${item.daysLeft.toFixed(1)} дн.`
                            : "запас не рассчитан"}
                        </span>
                      </div>
                    </div>
                ...
                <div className="bg-slate-50 rounded-xl p-8 text-center border border-slate-200 border-dashed">
                  <p className="text-sm text-slate-500">
                    Проблемных товаров категории A нет
                  </p>
                </div>
              )}
            </div>
