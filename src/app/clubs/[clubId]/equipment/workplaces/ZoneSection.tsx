...
const EQUIPMENT_TYPE_SORT_ORDER = [
  "PC",
  "CONSOLE",
  "TV",
  "MONITOR",
    ...
    <section className="space-y-4">
      <div className="group sticky top-0 z-10 flex items-center justify-between border-b bg-background px-1 py-2 sm:px-2">
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
          <h2 className="flex items-center gap-2.5 text-base font-black uppercase tracking-widest text-muted-foreground sm:gap-3 sm:text-lg">
            ...
            <Badge
              variant="secondary"
              className="border-none bg-accent px-2 text-muted-foreground"
            >
              {workstations.length}
            </Badge>
          </h2>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {workstations.map((ws) => {
          ...
          const wsOverdueMaintenanceCount =
            overdueMaintenanceCountByWorkstationId.get(ws.id) ?? 0;
          const wsServicedMaintenanceCount =
          ...
          const peripheralItems = sortedEquipment.filter(
            (i) =>
              i.id !== pcItem?.id && !componentItems.find((c) => c.id === i.id),
          );

          const mainDeviceCount =
            sortedEquipment.length - componentItems.length;
                    ...
                    <div>
                      ...
                      <div className="mt-0.5 flex flex-col gap-0.5">
                        ...
                        <div
                          ...
                          title={ws.assigned_user_name || "Не назначено"}
                        >
                          <User
                            className={cn(
                              "h-3 w-3",
                          ...
                          <span
                            className={cn(
                              "max-w-[100px] truncate text-[10px] font-medium",
                              ws.assigned_user_name
                                ? "text-primary"
                                : "text-muted-foreground/70",
                            )}
                      ...
                      {(() => {
                            ...
                            <div
                              ...
                              className={cn(
                                ...
                                itemIssueCount === 0 &&
                                  ...
                                  "border-l-[3px] border-l-slate-400 border-border bg-accent/70 pl-2.5",
                                isSubItem && "ml-4 bg-slate-50/50",
                              )}
                            >
                        ...
                        return (
                          <>
                            ...
                            {peripheralItems.map((item) => renderItemRow(item))}
                          </>
                        );
                      })()}
                    </div>
