...
const AssignEquipmentDialog = dynamic(() => import("./AssignEquipmentDialog"), {
  ssr: false,
});
const WorkplaceFormDialog = dynamic(() => import("./WorkplaceFormDialog"), {
  ssr: false,
});
...
export default function WorkplacesPage() {
  ...
  const syncWorkstationEquipmentCounts = useCallback(
    (
      previousWorkstationId: string | null | undefined,
      nextWorkstationId: string | null | undefined,
    ) => {
      ...
      setWorkstations((prev) =>
        prev.map((workstation) => {
          ...
          if (
            ...
            workstation.id === previousWorkstationId
          ) {
  ...
  const pendingUnassignEquipment = useMemo(() => {
    return pendingUnassignEquipmentId
  ...
  const pendingUnassignWorkstationName = useMemo(() => {
    ...
    return (
      workstations.find(
        (item) => item.id === pendingUnassignEquipment.workstation_id,
      )?.name ?? null
    );
    ...
    async (equipmentId: string) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          syncWorkstationEquipmentCounts(
            previousWorkstationId,
            updatedEquipment.workstation_id ?? null,
          );
        }
      } catch (error) {
  ...
  const activeIssueCountByWorkstationId = useMemo(() => {
    ...
    equipmentByWorkstationId.forEach((items, workstationId) => {
      const issuesCount = items.reduce(
        (total, item) =>
          total + (activeIssueCountByEquipmentId.get(item.id) ?? 0),
        0,
      );
      if (issuesCount > 0) {
  ...
  const overdueMaintenanceCountByWorkstationId = useMemo(() => {
    ...
    equipmentByWorkstationId.forEach((items, workstationId) => {
      const overdueCount = items.filter(
        (item) => maintenanceStatusByEquipmentId.get(item.id) === "overdue",
      ).length;
      if (overdueCount > 0) {
  ...
  const servicedMaintenanceCountByWorkstationId = useMemo(() => {
    ...
    equipmentByWorkstationId.forEach((items, workstationId) => {
      const servicedCount = items.filter(
        (item) => maintenanceStatusByEquipmentId.get(item.id) === "serviced",
      ).length;
      if (servicedCount > 0) {
  ...
  const disabledMaintenanceCountByWorkstationId = useMemo(() => {
    ...
    equipmentByWorkstationId.forEach((items, workstationId) => {
      const disabledCount = items.filter(
        (item) => maintenanceStatusByEquipmentId.get(item.id) === "disabled",
      ).length;
      if (disabledCount > 0) {
  ...
  const zoneIssueCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const issuesCount = (workstationsByZone.get(zone) ?? []).reduce(
        (total, workstation) =>
          total + (activeIssueCountByWorkstationId.get(workstation.id) ?? 0),
        0,
      );
  ...
  const zoneRepairCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const repairCount = (workstationsByZone.get(zone) ?? []).reduce(
        (total, workstation) => {
          const hasRepairEquipment = (
            equipmentByWorkstationId.get(workstation.id) ?? []
          ).some((item) => item.status === "REPAIR");
  ...
  const zoneEmptyCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const emptyCount = (workstationsByZone.get(zone) ?? []).filter(
        (workstation) => (workstation.equipment_count ?? 0) === 0,
      ).length;
      if (emptyCount > 0) {
  ...
  const zoneUnassignedCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const withoutResponsible = (workstationsByZone.get(zone) ?? []).filter(
        (workstation) => !workstation.assigned_user_id,
      ).length;
  ...
  const zoneOverdueMaintenanceCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const overdueCount = (workstationsByZone.get(zone) ?? []).reduce(
        (total, workstation) =>
          total +
          (overdueMaintenanceCountByWorkstationId.get(workstation.id) ?? 0),
        0,
      );
  ...
  const zoneServicedMaintenanceCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const servicedCount = (workstationsByZone.get(zone) ?? []).reduce(
        (total, workstation) =>
          total +
          (servicedMaintenanceCountByWorkstationId.get(workstation.id) ?? 0),
        0,
      );
  ...
  const zoneDisabledMaintenanceCountByName = useMemo(() => {
    ...
    for (const zone of zones) {
      const disabledCount = (workstationsByZone.get(zone) ?? []).reduce(
        (total, workstation) =>
          total +
          (disabledMaintenanceCountByWorkstationId.get(workstation.id) ?? 0),
        0,
      );
      ...
      return (
        <div className="text-center py-20 bg-slate-50 rounded-[40px] border-2 border-dashed border-slate-200">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Сначала настройте зоны клуба, после этого можно будет создавать
            рабочие места.
          </p>
    ...
    return zones.map((zone) => (
      <ZoneSection
        ...
        zoneAssignedUserName={
          zoneMetaByName.get(zone)?.assigned_user_name ?? null
        }
        ...
        zoneOverdueMaintenanceCount={
          zoneOverdueMaintenanceCountByName.get(zone) ?? 0
        }
        zoneServicedMaintenanceCount={
          zoneServicedMaintenanceCountByName.get(zone) ?? 0
        }
        zoneDisabledMaintenanceCount={
          zoneDisabledMaintenanceCountByName.get(zone) ?? 0
        }
  ...
  return (
    <PageShell maxWidth="5xl">
      <div className="space-y-8 pb-28 sm:pb-12">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between mb-8">
            <div className="min-w-0">
              ...
              <p className="text-slate-500 text-lg mt-2">
                Обзор зон, мест и подключенного оборудования
              </p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap lg:justify-end">
              ...
              <Button
                ...
                className="w-full bg-slate-900 text-white shadow-sm hover:bg-slate-800 disabled:opacity-60 sm:w-auto rounded-xl h-11 px-6 font-medium"
              >
        ...
        <div className="space-y-12">{zonesContent}</div>
