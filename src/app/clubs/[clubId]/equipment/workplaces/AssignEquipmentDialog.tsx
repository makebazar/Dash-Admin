...
interface AssignEquipmentDialogProps {
  ...
  renderEquipmentIcon: (type: string, typeIcon?: string | null) => ReactNode;
}

const VirtualizedAssignEquipmentList = memo(
  function VirtualizedAssignEquipmentList({
    items,
    renderEquipmentIcon,
    onAssignEquipment,
    ...
    const startIndex = Math.max(
      0,
      Math.floor(scrollTop / ASSIGN_DIALOG_ITEM_HEIGHT) -
        ASSIGN_DIALOG_OVERSCAN,
    );
    const endIndex = Math.min(
      items.length,
      Math.ceil(
        (scrollTop + ASSIGN_DIALOG_VIEWPORT_HEIGHT) / ASSIGN_DIALOG_ITEM_HEIGHT,
      ) + ASSIGN_DIALOG_OVERSCAN,
    );
      ...
      >
        <div style={{ height: totalHeight, position: "relative" }}>
          <div
            ...
            style={{
              transform: `translateY(${startIndex * ASSIGN_DIALOG_ITEM_HEIGHT}px)`,
            }}
          >
            {visibleItems.map((item) => (
              <div
                key={item.id}
                className="flex h-[70px] items-center justify-between rounded-xl border bg-card p-3 shadow-sm transition-all hover:border-primary/50"
              >
                <div className="flex min-w-0 items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-accent text-muted-foreground">
                    {renderEquipmentIcon(item.type, item.type_icon)}
                  </div>{" "}
                ...
                <Button
                  size="sm"
                  disabled={assigningEquipmentId !== null}
                  onClick={() => onAssignEquipment(item.id)}
                >
...
export default function AssignEquipmentDialog({
  ...
  const [assigningEquipmentId, setAssigningEquipmentId] = useState<
    string | null
  >(null);
  ...
  const normalizedEquipmentSearch = useMemo(
    () => deferredSearchEquipment.trim().toLowerCase(),
    [deferredSearchEquipment],
  );

  const availableEquipment = useMemo(() => {
    return equipment.filter((item) => {
      if (item.workstation_id || !canAssignEquipmentToWorkstation(item.status))
        return false;
      if (
        selectedEquipmentType !== "all" &&
        item.type !== selectedEquipmentType
      )
        return false;
      ...
      <DialogContent className="left-0 top-0 flex h-screen w-screen max-w-none translate-x-0 translate-y-0 flex-col overflow-hidden rounded-none border-0 p-0 [&>button]:hidden sm:left-1/2 sm:top-1/2 sm:h-[80vh] sm:w-[calc(100vw-1rem)] sm:max-w-180 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-lg sm:border">
        ...
        <div className="space-y-4 border-b bg-card p-4">
          ...
          <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
            ...
            {assignError ? (
              <span className="text-rose-600">{assignError}</span>
            ) : null}
          </div>
        </div>

        <div className="flex-1 bg-muted/50 p-3 pb-[calc(5.5rem+env(safe-area-inset-bottom))] sm:p-4 sm:pb-4">
          {availableEquipment.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="mt-1 text-center text-xs text-muted-foreground">
                Проверьте фильтры или убедитесь, что нужное устройство находится
                на складе.
              </p>
            </div>
          ) : (
