            ...
            <div style={{ height: totalHeight, position: "relative" }}>
                ...
                >
                    {visibleItems.map(item => (
                        <div key={item.id} className="flex h-[70px] items-center justify-between rounded-xl border bg-card p-3 shadow-sm transition-all hover:border-primary/50">
                            <div className="flex min-w-0 items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {item.identifier ? (
                                            <span className="hidden truncate text-[10px] text-muted-foreground/70 sm:inline">{item.identifier}</span>
                                        ) : null}
                                    </div>
                                </div>
                            </div>
            ...
            <DialogContent className="left-0 top-0 flex h-screen w-screen max-w-none translate-x-0 translate-y-0 flex-col overflow-hidden rounded-none border-0 p-0 [&>button]:hidden sm:left-1/2 sm:top-1/2 sm:h-[80vh] sm:w-[calc(100vw-1rem)] sm:max-w-[720px] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-lg sm:border">
                ...
                <div className="space-y-4 border-b bg-card p-4">
                    ...
                    <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
                        <span>Доступно на складе: <span className="font-medium text-foreground">{availableEquipment.length}</span></span>
                ...
                <div className="flex-1 bg-muted/50 p-3 pb-[calc(5.5rem+env(safe-area-inset-bottom))] sm:p-4 sm:pb-4">
                    {availableEquipment.length === 0 ? (
                        <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                            ...
                            <p className="text-sm font-medium text-foreground">Подходящее оборудование не найдено.</p>
