...
export default function ChecklistsPage({
  ...
  const [shiftReportFields, setShiftReportFields] = useState<
    ShiftReportField[]
  >([]);
  ...
  const filteredShifts = useMemo(() => {
    return shifts.filter((shift) => {
      const isIncoming =
        Boolean(shift.check_out) && shift.status !== "VERIFIED";
  ...
  const formatShiftCorrectionValue = (
          ...
          <TabsContent value="equipment">
            <div className="space-y-6">
              <Tabs
                value={equipmentTab}
                onValueChange={(v) =>
                  setEquipmentTab(v as "active" | "history")
                }
                className="w-full"
              >
                    ...
                    <div key={zoneName} className="space-y-4 pb-24">
                      ...
                      <div className="flex flex-col gap-3 sm:gap-3">
                        {zoneTasks.map((task) => {
                          ...
                          const isLaundryItem = isLaundryEquipmentType(
                            task.equipment_type,
                          );
                          const latestSubmission =
                            getLatestTaskSubmission(task);
                          const previousSubmission =
                            getPreviousTaskSubmission(task);
                                      ...
                                      <div className="mt-2 flex items-center justify-between gap-3 text-xs">
                                        ...
                                        {task.verification_status ===
                                          ...
                                          <div className="inline-flex max-w-full items-center text-[11px] font-medium leading-4 text-amber-700">
                                            На доработке {task.rework_days || 0}{" "}
                                            дн.
                                          </div>
                                        )}
                                        {task.verification_status ===
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3 pt-2">
                                          {isLaundryItem && (
                                            <Button
                                              ...
                                              onClick={() =>
                                                handleSendToLaundry(task)
                                              }
                                              disabled={
                                                isSubmittingTask ||
                                                Boolean(task.laundry_request_id)
                                              }
                                            >
              ...
              {isShiftsLoading ? (
                ...
                <div className="space-y-3 pb-24">
                  {filteredShifts.map((shift) => {
                    const cashIncome = getShiftMetricValue(
                      shift,
                      "cash_income",
                    );
                    const cardIncome = getShiftMetricValue(
                      shift,
                      "card_income",
                    );
                          ...
                          <div className="space-y-4">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-muted-foreground">
                                  ...
                                  <span>
                                    ...
                                    {shift.check_out
                                      ? format(
                                          new Date(shift.check_out),
                                          "HH:mm",
                                        )
                                      : "..."}
                                  </span>
                                  <span>
                                    {Number(shift.total_hours || 0).toFixed(1)}{" "}
                                    ч
                                  </span>
                                </div>
                              </div>
                            ...
                            {shift.has_owner_corrections &&
                              ...
                              shift.owner_correction_changes.length > 0 ? (
                                <div className="rounded-lg border border-orange-200 bg-orange-50 px-3 py-3 text-sm text-orange-900">
                                  ...
                                  {shift.owner_correction_changes.length >
                                    3 && (
                                    <div className="mt-2 text-xs text-orange-700">
                                      И еще{" "}
                                      {shift.owner_correction_changes.length -
                                        3}{" "}
                                      правки в карточке смены
                                    </div>
                                  )}
                                </div>
                              ) : (
                                <div className="rounded-lg border border-dashed border-orange-200 bg-orange-50 px-3 py-2 text-sm text-orange-800">
                                  Детализация правок для этой смены не
                                  сохранена, потому что она была изменена до
                                  добавления diff.
                                </div>
                              ))}
                            ...
                            <div className="flex flex-col-reverse gap-2 border-t pt-4 sm:flex-row sm:justify-end">
                              <Button
                                ...
                                onClick={() =>
                                  router.push(
                                    `/clubs/${clubId}/shifts/${shift.id}?from=reviews`,
                                  )
                                }
                              >
                              ...
                              {shift.status !== "VERIFIED" && (
                                <Button
                                  ...
                                  onClick={() =>
                                    handleVerifyShiftForReview(shift)
                                  }
          ...
          <TabsContent value="checklists">
            <div className="space-y-6">
              <Tabs
                value={checklistsTab}
                onValueChange={(v) =>
                  setChecklistsTab(v as "active" | "history")
                }
                className="w-full"
              >
                <TabsList className="flex h-auto w-full justify-start gap-6 overflow-x-auto rounded-none bg-transparent p-0 mb-6">
                ...
                {isLoading ? (
                  ...
                  filteredChecklists.map((evaluation) => {
                    const percent = Math.round(
                      (evaluation.total_score / (evaluation.max_score || 100)) *
                        100,
                    );
                    return (
                      <div
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {checklistsTab === "history" && (
                                <Button
                                  ...
                                  onClick={(e) =>
                                    handleRestoreChecklist(evaluation, e)
                                  }
                                  disabled={
                                    restoringChecklistId === evaluation.id
                                  }
                                >
                              ...
                              <Button
                                ...
                                onClick={(e) =>
                                  handleDeleteChecklist(evaluation, e)
                                }
        ...
        >
          <DialogContent>
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setRestoreChecklistTarget(null)}
              >
                Отмена
              </Button>
              <Button
