...
export default function EmployeesPage({
  ...
  const [editAddShiftRoleId, setEditAddShiftRoleId] = useState<number | null>(
    null,
  );
  ...
  const [employeeToDismiss, setEmployeeToDismiss] = useState<string | null>(
    null,
  );
  ...
  const fetchData = async (id: string) => {
    try {
      const [employeesRes, rolesRes, schemesRes, applicationsRes] =
        await Promise.all([
      ...
      const [employeesData, rolesData, schemesData, applicationsData] =
        await Promise.all([
          employeesRes.json(),
          rolesRes.json(),
          schemesRes.json(),
  ...
  const handleAssignRoleScheme = async (
    employeeId: string,
    roleId: number,
    value: string,
  ) => {
    try {
      ...
      const res = await fetch(
        `/api/clubs/${clubId}/employees/${employeeId}/role-salary`,
        {
          method: "POST",
      ...
      const schemeName = schemeId
        ? salarySchemes.find((s) => s.id === schemeId)?.name || null
        : null;
      setEmployees((prev) =>
        prev.map((emp) => {
          ...
          return {
            ...
            role_salary_overrides: [
              ...filtered,
              { role_id: roleId, scheme_id: schemeId, scheme_name: schemeName },
            ],
          };
        }),
      );
  ...
  const handleConfirmDismiss = async () => {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/employees/${employeeToDismiss}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleEditEmployee = async (employee: Employee) => {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/employees/${employee.id}/roles`,
        { cache: "no-store" },
      );
      const data = await res.json();
  ...
  const handleUpdateEmployee = async (e: React.FormEvent) => {
    ...
    try {
      const primaryRoleId =
        editShiftRoleIds.length > 0 ? editShiftRoleIds[0] : null;
      const body: any = {
      ...
      const res = await fetch(
        `/api/clubs/${clubId}/employees/${selectedEmployee.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        await fetch(
          `/api/clubs/${clubId}/employees/${selectedEmployee.id}/roles`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  const handleConfirmRestore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/clubs/${clubId}/employees/${employeeToRestore.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((emp) => {
      ...
      const matchesStatus =
        activeTab === "active"
          ? emp.is_active && !emp.dismissed_at
          : !emp.is_active || emp.dismissed_at;
  ...
  const stats = useMemo(() => {
    const activeEmployees = employees.filter(
      (employee) => employee.is_active && !employee.dismissed_at,
    ).length;
    ...
    <PageShell maxWidth="5xl">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between mb-12">
        <div className="space-y-3">
          ...
          <p className="text-slate-500 text-lg">Управление персоналом</p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
            ...
            >
              <TabsList className="grid w-full grid-cols-2 bg-slate-100/50 p-1 rounded-xl">
                <TabsTrigger
                  value="active"
                  className="text-xs font-semibold rounded-lg data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm"
                >
                  Активные
                </TabsTrigger>
                <TabsTrigger
                ...
                <Table>
                  <TableHeader className="bg-slate-50/50">
                    <TableRow className="hover:bg-transparent border-slate-100">
                      ...
                      <TableHead className="text-xs font-bold uppercase tracking-widest text-slate-500 py-4 text-right">
                        Действия
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-1.5">
                            ...
                            employee.shift_roles.length > 0
                              ...
                              return (
                                <div
                                  key={`${employee.id}-${r.role_id}-${idx}`}
                                  className="flex items-center gap-2"
                                >
                                  <Badge
                                    variant="outline"
                                    className="bg-slate-50 text-slate-600 border-slate-200 font-bold text-[10px] uppercase tracking-wider px-2 py-0.5"
                                  >
                                    {r.role_name}
                                  </Badge>
                                </div>
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-2 max-w-55">
                            ...
                            employee.shift_roles.length > 0
                              ...
                              .filter((r) => r.role_id > 0)
                              .map((r, idx) => {
                                const override = (
                                  employee.role_salary_overrides || []
                                ).find((x) => x.role_id === r.role_id);
                                const currentValue = override
                                  ? override.scheme_id === null
                                    ? "none"
                                    : String(override.scheme_id)
                                  : employee.salary_scheme_id
                                    ? String(employee.salary_scheme_id)
                                    : "none";
                                return (
                                  <Select
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 rounded-full hover:bg-slate-100 hover:text-slate-900"
                              >
                            ...
                            <DropdownMenuContent
                              align="end"
                              className="w-48 rounded-xl border-slate-200 shadow-xl shadow-slate-900/5"
                            >
                              <DropdownMenuItem
                                onClick={() => handleEditEmployee(employee)}
                                className="text-xs font-medium cursor-pointer py-2 gap-2"
                              >
                              ...
                              {employee.is_active && !employee.dismissed_at ? (
                                <DropdownMenuItem
                                  onClick={() =>
                                    handleDismissEmployee(employee.id)
                                  }
                                  className="text-xs font-medium cursor-pointer py-2 gap-2 text-rose-600 focus:text-rose-600 focus:bg-rose-50"
                                >
                                ...
                                <DropdownMenuItem
                                  ...
                                  className="text-xs font-medium cursor-pointer py-2 gap-2 text-emerald-600 focus:text-emerald-600 focus:bg-emerald-50"
                                >
                  ...
                  >
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 rounded-full -mr-2 -mt-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100"
                          >
                            <MoreHorizontal className="h-4 w-4" />
                          </Button>
                        </DropdownMenuTrigger>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(employee.shift_roles && employee.shift_roles.length > 0
                        ...
                        return (
                          <div
                            key={`${employee.id}-m-${r.role_id}-${idx}`}
                            className="flex items-center gap-2"
                          >
                            <Badge
                              variant="outline"
                      ...
                      {employee.dismissed_at ? (
                        <Badge
                          variant="outline"
                          className="bg-rose-50 text-rose-600 border-rose-200 font-bold text-[10px] uppercase tracking-wider px-2 py-0.5"
                        >
                          Уволен:{" "}
                          {new Date(employee.dismissed_at).toLocaleDateString(
                            "ru-RU",
                        ...
                        <Badge
                          variant="outline"
                          className="bg-emerald-50 text-emerald-600 border-emerald-200 font-bold text-[10px] uppercase tracking-wider px-2 py-0.5"
                        >
                          Нанят:{" "}
                          {new Date(employee.hired_at).toLocaleDateString(
                            "ru-RU",
                    ...
                    <div className="pt-3 border-t border-slate-100 space-y-2">
                      ...
                      {(employee.shift_roles && employee.shift_roles.length > 0
                        ...
                        .map((r, idx) => {
                          ...
                          return (
                            <div
                              key={`${employee.id}-m-scheme-${r.role_id}-${idx}`}
                              className="space-y-1"
                            >
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-xl border-t border-slate-200 md:hidden z-50 flex gap-2 pb-[calc(1rem+env(safe-area-inset-bottom))]">
        <Button
          asChild
          variant="outline"
          className="flex-1 bg-white border-slate-200 text-slate-700 h-12 rounded-xl font-medium"
        >
          <Link href={`/clubs/${clubId}/employees/recruitment/applications`}>
        ...
        <Button
          onClick={() => setIsModalOpen(true)}
          className="flex-1 bg-slate-900 text-white hover:bg-slate-800 h-12 rounded-xl font-medium"
        >
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="w-screen h-dvh max-w-none rounded-none border border-slate-200 shadow-2xl bg-white p-6 overflow-y-auto sm:w-full sm:h-auto sm:max-w-106.25 sm:rounded-3xl">
          <DialogHeader className="space-y-3">
            <div className="h-12 w-12 rounded-2xl bg-slate-100 flex items-center justify-center">
          ...
          <form
            ...
            className="space-y-5 pt-4 pb-24 sm:pb-0"
          >
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-500">
                      Порядок сверху вниз = приоритет по умолчанию
                    </p>
                  </div>
                  {editShiftRolesLoading && (
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 border border-slate-200">
                <div className="space-y-0.5">
                  <Label
                    ...
                    className="text-[10px] font-bold uppercase tracking-wider text-slate-900"
                  >
                    График работы
                  </Label>
                  <p className="text-[10px] text-slate-500">
                    Отображать сотрудника в сетке расписания
                  </p>
                </div>
                <Switch
                  id="showInSchedule"
            ...
            <div className="fixed inset-x-0 bottom-0 z-10 border-t bg-white/95 px-4 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] backdrop-blur supports-backdrop-filter:bg-white/80 sm:hidden">
              <div className="mx-auto flex max-w-106.25 gap-2">
                <Button
                  type="button"
                  variant="outline"
          ...
          <div className="py-4 space-y-4">
            ...
            <div className="bg-rose-50 border border-rose-100 p-4 rounded-xl space-y-2">
              <p className="text-[11px] text-rose-700 leading-relaxed font-medium">
                С этой даты сотрудник больше не сможет выходить в смены, но его
                история работы останется в архиве.
              </p>
            </div>
          </div>
        ...
        <DialogContent className="sm:max-w-100 rounded-3xl border border-slate-200 shadow-2xl bg-white p-6">
          <DialogHeader className="space-y-3">
            ...
            <div>
              ...
              <DialogDescription className="text-sm font-medium text-slate-500">
                Вы собираетесь восстановить{" "}
                <strong>{employeeToRestore?.full_name}</strong>
              </DialogDescription>
            </div>
          </DialogHeader>
          ...
          <div className="py-4">
            <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl space-y-2">
              <p className="text-[11px] text-emerald-700 leading-relaxed font-medium">
                После восстановления сотрудник снова сможет входить в систему и
                будет отображаться в списке активных сотрудников.
              </p>
            </div>
          </div>
