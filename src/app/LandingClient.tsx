...
export default function LandingClient({ plans }: { plans: any[] }) {
  return (
    <div className="min-h-screen bg-black text-white selection:bg-purple-500/30 font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-6 mix-blend-difference">
        ...
        <div className="flex items-center gap-6 text-sm font-medium">
          <Link
            href="/login"
            className="hover:text-white/70 transition-colors hidden sm:block"
          >
            Войти
          </Link>
          <Button
      ...
      {/* Hero */}
      <section className="relative min-h-svh flex flex-col justify-end pb-16 md:pb-24 px-6 md:px-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl">
          ...
          >
            <p className="text-xl md:text-2xl text-gray-400 max-w-xl leading-snug">
              Смена закрывается без расхождений. Техника обслуживается по
              графику. Качество контролируется чеклистами с фото, а все
              регламенты собраны в единой базе знаний.
            </p>
            <Button
              asChild
              size="lg"
              className="h-14 px-8 rounded-full bg-white text-black hover:bg-gray-200 text-lg font-medium group shrink-0"
            >
              <Link href="/login">
          ...
          <div className="lg:w-1/3">
            <div className="sticky top-32">
              ...
              <p className="text-gray-400 text-lg lg:text-xl leading-relaxed">
                Мы собрали то, где чаще всего теряются деньги и нервы. Больше
                никаких разрозненных таблиц и потерянных инструкций в чатах.
              </p>
            </div>
          </div>
      ...
      <section className="py-24 md:py-40 px-6 md:px-12 border-t border-white/10">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-20 md:mb-32">
            Для кого это
          </h2>
      ...
      {/* Tariffs */}
      <section className="py-24 md:py-40 px-6 md:px-12 bg-black border-t border-white/10">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 md:mb-24 gap-8">
            <div>
              ...
              <p className="text-gray-400 text-lg lg:text-xl max-w-xl mb-6">
                Единый тариф со всеми функциями. Выбирайте период оплаты — чем
                дольше, тем выгоднее.
              </p>
              <div className="inline-flex items-center gap-2 text-purple-400 font-medium bg-purple-500/10 border border-purple-500/20 px-4 py-2 rounded-xl text-sm lg:text-base">
                <Zap className="w-4 h-4 fill-current" />
                <span>Скидка 50% на второй и последующие клубы</span>
              </div>
            </div>
            <div className="text-left md:text-right shrink-0">
              <div className="inline-block px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm font-medium">
                14 дней бесплатно для новых клубов
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {plans.map((plan) => (
              <PricingCard
                key={plan.id}
                title={plan.name}
                price={`${new Intl.NumberFormat("ru-RU").format(Number(plan.price_amount))} ₽`}
      ...
      <section className="py-32 md:py-56 px-6 md:px-12 bg-black flex flex-col items-center justify-center text-center border-t border-white/10">
        ...
        >
          ...
          <p className="text-xl md:text-2xl text-gray-400 mb-12 max-w-2xl mx-auto leading-relaxed">
            14 дней полного доступа. Регистрация по телефону занимает считанные
            минуты.
          </p>
          <Button
            asChild
            size="lg"
            className="h-16 px-12 rounded-full bg-white text-black hover:bg-gray-200 text-xl font-medium"
          >
...
function FeatureRow({
  ...
  return (
    ...
    >
      ...
      <div>
        <h3 className="text-2xl md:text-4xl font-semibold mb-4 md:mb-6 group-hover:text-purple-400 transition-colors tracking-tight">
          {title}
        </h3>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline gap-1 mb-2">
        <span className="text-3xl lg:text-4xl font-bold tracking-tight whitespace-nowrap">
          {price}
        </span>
      </div>
      <p
